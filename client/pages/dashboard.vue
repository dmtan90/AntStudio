    ...
    <div class="dashboard-container">
      ...
      <div class="stats-grid">
        ...
        <GCard class="stat-card plan-card">
          ...
          <GButton type="text" size="sm" @click="handleCommand('profile')" class="upgrade-btn">{{ t('dashboard.upgrade') }}</GButton>
        </GCard>
      </div>
      ...
      <div class="projects-section">
        ...
        <div v-else class="empty-state">
          <GCard class="empty-card">
            ...
            <GButton type="primary" @click="navigateTo('/projects/new')">{{ t('dashboard.createProject') }}</GButton>
          </GCard>
        </div>
      </div>
    </div>
...
const handleCommand = (cmd: string) => {
  if (cmd === 'profile') {
    // This will be handled by AppSidebar's AccountDialog
    // For now, just navigate to account section
  }
}
...
const fetchProjects = async () => {
  try {
    const token = localStorage.getItem('auth-token')
    const { data } = await $fetch('/api/projects?limit=20', {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
