<template>
  <div class="resources-page">
    ...
    <GDialog
      ...
      width="500px"
    >
      <div class="upload-area" v-loading="isUploading">
        <input
          ref="fileInput"
          type="file"
          multiple
        ...
        <div class="upload-placeholder" @click="fileInput?.click()">
          ...
          <p>{{ isUploading ? 'Uploading...' : 'Click to upload or drag files here' }}</p>
        </div>
      </div>
    </GDialog>
...
const { resources, loading } = storeToRefs(mediaStore)
...
const fetchResources = () => {
  mediaStore.fetchMedia(currentFileType.value !== 'all' ? currentFileType.value : undefined)
}
...
const deleteResource = async (resource: any) => {
  ...
  try {
    await mediaStore.deleteMedia(resource._id)
