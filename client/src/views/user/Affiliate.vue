          ...
          <section class="glass-card p-8 rounded-2xl border border-white/5">
           ...
           <section class="glass-card p-6 rounded-2xl border border-white/5">
              ...
              <div v-if="recentCommissions.length === 0" class="text-center py-8 opacity-40">
                 <p class="text-xs">No commissions yet. Start sharing your referral link!</p>
              </div>
              <div v-else class="space-y-2">
                 <div v-for="commission in recentCommissions" :key="commission.id" class="commission-item p-4 bg-white/5 rounded-xl border border-white/5 flex justify-between items-center">
                    <div class="flex items-center gap-4">
                       ...
                       <div>
                          <p class="text-xs font-bold">{{ commission.status === 'paid' ? 'Commission Paid' : 'Commission Approved' }}</p>
                          <p class="text-[10px] opacity-40">User: {{ commission.referredUser }} • {{ new Date(commission.convertedAt).toLocaleDateString() }}</p>
                       </div>
                    </div>
       ...
       <div class="col-span-12 lg:col-span-4 space-y-6">
          <section class="glass-card p-8 rounded-3xl border border-blue-500/20 bg-gradient-to-br from-blue-600/10 to-transparent">
             ...
             <div class="text-center mb-8">
              ...
              <button class="primary-btn w-full py-4 text-xs font-black mb-4" @click="requestPayout" :disabled="!affiliate || affiliate.balance.unpaid <= 0">REQUEST PAYOUT</button>
...
const statCards = computed(() => ({
  'Total Traffic': formatLargeNumber(affiliate.value?.metrics.totalClicks || 0),
  'Conversions': formatLargeNumber(affiliate.value?.metrics.conversions || 0),
  'Revenue Generated': formatCurrency(affiliate.value?.metrics.totalRevenueGenerated || 0),
  'Conversion Rate': affiliate.value?.metrics.totalClicks > 0 
