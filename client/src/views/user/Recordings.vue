    ...
    <div v-else class="recordings-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
       <div v-for="rec in recordings" :key="rec.id" class="rec-card glass-card group">
          <div class="thumbnail-area">
             ...
             <div class="overlay">
                <button class="play-btn" @click="previewRec(rec)"><play theme="filled" /></button>
             </div>
          </div>
...
const fetchData = async () => {
    // Mocking recordings for now
    setTimeout(() => {
        recordings.value = [
          { 
            id: '1', title: 'Studio Session: AI Keynote', date: 'JAN 24, 2026', duration: '12:45', 
            thumbnail: 'https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?auto=format&fit=crop&w=400',
            quality: '4K', aiEffects: 3, scenes: 5, videoUrl: '#'
          },
          { 
            id: '2', title: 'Morning Live: Cyberpunk Vlog', date: 'JAN 22, 2026', duration: '08:20', 
            thumbnail: 'https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&w=400',
            quality: '1080P', aiEffects: 1, scenes: 2, videoUrl: '#'
          },
          { 
            id: '3', title: 'Tech Review: AntFlow Features', date: 'JAN 20, 2026', duration: '20:15', 
            thumbnail: 'https://images.unsplash.com/photo-1485827404703-89b55fcc595e?auto=format&fit=crop&w=400',
            quality: '4K', aiEffects: 5, scenes: 8, videoUrl: '#'
          }
        ];
        loading.value = false;
    }, 1000);
...
.header-stats {
  .stat {
    ...
    .label { font-[10px] font-black uppercase opacity-30 tracking-widest; }
    .value { font-size: 18px; font-weight: 800; color: #3b82f6; }
  }
}
...
.rec-card {
  ...
  .quality-tag {
     background: #1e1b4b; color: #818cf8; font-size: 9px; font-weight: 900;
     padding: 2px 6px; border-radius: 4px; border: 1px solid rgba(129, 140, 248, 0.3);
  }
  
  .meta-item { display: flex; align-items: center; font-size: 11px; opacity: 0.5; font-weight: 700; }
  
  .action-btn {
     height: 36px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1);
     background: rgba(255,255,255,0.05); color: #fff; font-size: 11px; font-weight: 800;
     @include flex-center; cursor: pointer; transition: 0.2s;
     &:hover { background: #fff; color: #000; border-color: #fff; }
     &.secondary { width: 36px; flex: none; }
  }
}
</style>
