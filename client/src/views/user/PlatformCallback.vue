    ...
    <div class="callback-page flex items-center justify-center h-screen bg-[#0a0a0a] font-sans">
        <div class="text-center animate-in fade-in zoom-in duration-300 relative z-10 glass-panel p-12">
            <div v-if="loading">
                ...
                <h2 class="text-2xl font-black text-white mb-2 tracking-tight">{{ t('platformCallback.connecting', { name: platformName }) }}</h2>
                <p class="text-gray-400 font-medium">{{ t('platformCallback.waitDesc') }}</p>
            </div>
            <div v-else-if="error">
                ...
                <h2 class="text-2xl font-black text-white mb-2 tracking-tight">{{ t('platformCallback.error.title') }}</h2>
                <p class="text-red-400 mb-8 font-medium">{{ error }}</p>
                <button class="primary-btn" @click="$router.push(returnUrl)">{{ t('platformCallback.error.action') }}</button>
            </div>
        </div>
    </div>
</template>
