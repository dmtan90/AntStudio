<template>
    <div class="influencer-library min-h-screen bg-[#0a0a0c] text-white">
        ...
        <header class="relative py-20 px-8 overflow-hidden border-b border-white/5">
            ...
            <div class="absolute -bottom-24 -right-24 w-96 h-96 bg-blue-500/5 rounded-full blur-[80px]"></div>
            ...
            <div class="max-w-7xl mx-auto relative z-10">
                ...
                <div class="flex flex-wrap gap-4 items-center">
                    ...
                    <button @click="() => fetchInfluencers()" :disabled="isLoading" class="group px-8 py-4 bg-white/5 border border-white/10 text-white rounded-2xl font-black hover:bg-white/10 hover:scale-105 transition-all flex items-center gap-3 relative overflow-hidden disabled:opacity-50">
                        ...
                        {{ isLoading ? t('influencers.syncing') : t('influencers.sync') }}
                    </button>
                </div>
            </div>
        </header>
        ...
        <main class="max-w-7xl mx-auto py-16 px-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                ...
                <div class="relative group min-w-[320px]">
                    ...
                    <input v-model="filters.search" :placeholder="t('influencers.search')" class="w-full pl-14 pr-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-purple-500/50 focus:bg-white/[0.08] transition-all outline-none text-base" />
                </div>
            </div>

            <!-- Category Filters -->
            <div v-if="activeTab !== 'my-influencers'" class="mb-12 flex flex-wrap items-center gap-3">
            ...
            <!-- My Influencers Grid -->
            <div v-else-if="activeTab === 'my-influencers' && influencers.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                <InfluencerCard v-for="influencer in influencers" :key="influencer._id" :influencer="influencer" @click="selectInfluencer(influencer)" @dashboard="openDashboard(influencer)" @delete="confirmDelete(influencer)" />
            </div>
            ...
            <div v-else class="text-center py-32 border border-dashed border-white/10 rounded-3xl text-center">
                ...
                <h3 class="text-xl font-black mb-2">{{ t('influencers.empty.noInfluencers') }}</h3>
                <p class="text-gray-500">
                    {{ activeTab === 'my-influencers' ? t('influencers.empty.myInfluencersDesc') : t('influencers.empty.presetsDesc') }}
                </p>
            </div>

            <!-- Pagination (Mocked for now as Influencer store pagination might differ) -->
            <div v-if="activeTab === 'my-influencers' && influencers.length > 0 && pagination.total > filters.limit" class="mt-12 w-full flex justify-center glass-pagination">
...
import PresetCard from '@/components/influencer/PresetCard.vue';
import InfluencerStudio from '@/components/influencer/InfluencerStudio.vue';
import InfluencerDashboard from '@/components/influencer/InfluencerDashboard.vue';

// Import presets
import influencerPresetsData from '@/data/influencer-presets.json';

const influencerStore = useInfluencerStore();
const { influencers, isLoading } = storeToRefs(influencerStore);
const { t } = useI18n()

// Mock pagination if store doesn't have it yet, but keep reactive
const pagination = ref({ total: 0 });

const activeInfluencer = ref<any>(null);
...
const presets = ref({
    live2d: influencerPresetsData.live2d || [],
    static: influencerPresetsData.static || []
});

const categories = ref(influencerPresetsData.categories || []);
...
const fetchInfluencers = async (page = 1) => {
    const res = await influencerStore.fetchInfluencers(page, filters.value.limit);
...
const selectInfluencer = (influencer: any) => {
    activeInfluencer.value = influencer;
    showUpdate.value = true;
};

const openDashboard = (influencer: any) => {
    activeInfluencer.value = influencer;
    showDashboard.value = true;
};

const confirmDelete = async (influencer: any) => {
    if (confirm(t('influencers.confirmDelete', { name: influencer.identity.name }))) {
        await influencerStore.deleteInfluencer(influencer.entityId);
    }
};

const usePreset = async (preset: any) => {
    try {
        toast.loading(t('influencers.toasts.cloning'));
        // await influencerStore.clonePreset(preset); 
        toast.success(t('influencers.toasts.cloneSuccess'));
        activeTab.value = 'my-influencers';
        fetchInfluencers(1);
    } catch (e: any) {
        console.error('Failed to clone preset:', e);
        toast.error(t('influencers.toasts.cloneFailed'));
    }
};

const previewPreset = (preset: any) => {
    console.log('Preview preset:', preset);
    toast.info(t('influencers.toasts.previewSoon'));
};
...
watch(() => filters.value.search, () => {
    if (activeTab.value === 'my-influencers') {
        fetchInfluencers(1);
    }
});

onMounted(() => fetchInfluencers(1));
</script>
