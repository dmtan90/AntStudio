<template>
  <div class="infra-health p-6 animate-in">
    <header class="page-header mb-10 flex justify-between items-start">
      ...
      <div class="system-badge flex items-center gap-3 bg-green-500/10 px-4 py-2 rounded-full border border-green-500/20">
    ...
    <div class="regions-grid grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
       <div v-for="region in regions" :key="region.id" class="region-card glass-card p-6 rounded-2xl border border-white/5 relative overflow-hidden">
          ...
          <div class="stats-row flex gap-6">
             <div class="stat">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
       <section class="glass-card p-8 rounded-3xl border border-white/5">
          ...
          <div class="nodes-list space-y-4">
             <div v-for="node in dbNodes" :key="node.id" class="node-item p-4 bg-white/5 rounded-xl border border-white/5 flex justify-between items-center">
       ...
       <section class="glass-card p-8 rounded-3xl border border-white/5">
          <div class="flex justify-between items-center mb-8">
             ...
             <span class="text-[8px] px-3 py-1 rounded-full bg-orange-500/10 text-orange-500 font-bold uppercase border border-orange-500/20">Standby Ready</span>
          </div>

          <p class="text-xs text-gray-400 mb-8 leading-relaxed">Automated disaster recovery is active. If the Primary Region (Asia-East-1) fails, traffic is automatically re-routed to US-West-2 within 45 seconds.</p>
          
          <div class="flex gap-4">
             <button class="primary-btn flex-1 py-4 text-[10px] font-black" @click="triggerManualFailover">TEST FAILOVER</button>
             <button class="secondary-btn flex-1 py-4 text-[10px] font-black" @click="syncBackups">VALIDATE BACKUPS</button>
          </div>
          ...
          <div class="mt-10 pt-10 border-t border-white/5">
             ...
             <div class="log-stream h-32 overflow-y-auto font-mono text-[9px] text-gray-500 space-y-1">
                <p v-for="l in heartbeats" :key="l.t">[{{ l.t }}] INFRA_PULSE: Region {{ l.region }} healthy. Load: {{ l.load }}%</p>
             </div>
          </div>
       </section>
    </div>
...
const regions = ref([
    { id: '1', name: 'ASIA-EAST-1 (ORIGIN)', status: 'PRIMARY', latency: 42, load: 38 },
    { id: '2', name: 'US-WEST-2', status: 'STANDBY', latency: 140, load: 12 },
    { id: '3', name: 'EU-CENTRAL-1', status: 'STANDBY', latency: 184, load: 8 }
]);

const dbNodes = ref([
    { id: 'master', host: 'db-master-01.antflow.internal', role: 'WRITER', uptime: '14d 22h' },
    { id: 'read1', host: 'db-replica-01.antflow.internal', role: 'READER', uptime: '14d 22h' },
    { id: 'read2', host: 'db-replica-02.antflow.internal', role: 'READER', uptime: '8d 04h' }
]);

const heartbeats = ref([
    { t: '10:02:15', region: 'Asia-East-1', load: 38 },
    { t: '10:01:15', region: 'Asia-East-1', load: 41 },
    { t: '10:00:15', region: 'Asia-East-1', load: 35 }
]);
