<template>
    <div class="release-hub p-8 animate-in">
        <header class="flex justify-between items-start mb-12">
            <div>
                <h1 class="text-4xl font-black tracking-tighter text-white mb-2">{{ $t('admin.release.title') }}</h1>
                <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-500">{{ $t('admin.release.subtitle') }}
                </p>
            </div>
        ...
        <div v-if="showUpload"
            ...
            <div class="bg-[#0a0a0c] border border-white/10 rounded-3xl w-full max-w-lg p-8">
                <h3 class="text-xl font-black mb-6">{{ $t('admin.release.deployVersion') }}</h3>
                <div class="space-y-4">
                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-500">{{ $t('admin.release.semanticVersion') }}</label>
                        <input v-model="form.version" :placeholder="$t('admin.release.placeholderVersion')"
                    ...
                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-500">{{ $t('admin.release.releaseChannel') }}</label>
                        <select v-model="form.channel"
                            class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 outline-none">
                            <option value="stable">{{ $t('admin.release.channels.stable') }}</option>
                            <option value="beta">{{ $t('admin.release.channels.beta') }}</option>
                            <option value="nightly">{{ $t('admin.release.channels.nightly') }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-500">{{ $t('admin.release.tacticalNotes') }}</label>
                        <textarea v-model="form.releaseNotes" rows="4" :placeholder="$t('admin.release.placeholderNotes')"
                    ...
                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-500">{{ $t('admin.release.artifactUrl') }}</label>
                    ...
                    <div class="flex justify-end gap-2 mt-6">
                        <button @click="showUpload = false"
                            class="px-4 py-2 text-xs font-bold text-gray-400 hover:text-white">{{ $t('common.cancel') }}</button>
                        <button @click="publishRelease"
                            class="px-6 py-2 bg-blue-600 rounded-xl text-xs font-black uppercase hover:bg-blue-500">{{ $t('admin.release.deploy') }}</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div v-for="rel in releases" :key="rel._id"
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            <h3 class="font-bold text-white">{{ rel.channel.toUpperCase() }} {{ $t('admin.release.channelSuffix') }}</h3>
                ...
                <div class="flex items-center gap-4">
                    <a :href="rel.downloadUrl" target="_blank"
                        ...
                        <download theme="outline" /> {{ $t('admin.release.downloadArtifact') }}
                    </a>
                </div>
            </div>
        </div>
...
const fetchReleases = async () => {
    try {
        ...
        console.error(t('admin.release.toasts.fetchFailed'), e);
    }
};
