<template>
   <div class="live-studio dark-theme">
      ...
      <main class="studio-main">
         ...
         <StudioStage :active-f-x="activeFX" :active-explosions="activeExplosions">
            <template #canvas>
               <div class="relative w-full h-full overflow-hidden rounded-[2.5rem] border border-white/10 shadow-2xl transition-all duration-700"
                  ...
                  <div v-if="studioStore.streamingContext === 'general'"
                     class="audio-visual-cue absolute bottom-8 left-8 flex items-end gap-1 px-4 py-2 bg-black/40 backdrop-blur-md rounded-2xl border border-white/5 z-40 pointer-events-none">
                ...
                <transition name="context-blur" mode="out-in">
                   <Component :is="contextComponent" v-if="contextComponent" class="z-30" @highlight-product="handleProductHighlight" />
                </transition>
               </div>
            </template>
            <template #controls>
         ...
         <DirectorNotes v-if="studioStore.streamingContext !== 'general'" />
      </main>
      ...
      <!-- Influencer Performance Overlay (Embedded) -->
...
// Quest Overlay removed (now in-canvas)
...
import { influencerDubber } from '@/utils/ai/InfluencerDubber';
...
import { audienceQuestService } from '@/utils/ai/AudienceQuestService';
import { audienceIntelligenceService } from '@/utils/ai/AudienceIntelligenceService';
...
import MusicSelectionDialog from '@/components/influencer/MusicSelectionDialog.vue';
import InfluencerMusicPerformance from '@/components/influencer/InfluencerMusicPerformance.vue';
import PerformanceOverlay from '@/components/studio/overlays/PerformanceOverlay.vue';
import StageLyricsOverlay from '@/components/influencer/StageLyricsOverlay.vue';
import NeuralDashboard from '@/components/studio/NeuralDashboard.vue';
import SupervisorDock from '@/components/studio/SupervisorDock.vue';

// Context Overlays
import SalesOverlay from '@/components/studio/overlays/SalesOverlay.vue';
import TalkShowOverlay from '@/components/studio/overlays/TalkShowOverlay.vue';
import NewsOverlay from '@/components/studio/overlays/NewsOverlay.vue';
import MusicOverlay from '@/components/studio/overlays/MusicOverlay.vue';
import GameShowOverlay from '@/components/studio/overlays/GameShowOverlay.vue';
import GameStreamingOverlay from '@/components/studio/overlays/GameStreamingOverlay.vue';
import SportOverlay from '@/components/studio/overlays/SportOverlay.vue';
import EducationOverlay from '@/components/studio/overlays/EducationOverlay.vue';
import CommentaryOverlay from '@/components/studio/overlays/CommentaryOverlay.vue';
import GeneralOverlay from '@/components/studio/overlays/GeneralOverlay.vue';
import PostShowOverlay from '@/components/studio/overlays/PostShowOverlay.vue';
...
const contextComponent = computed(() => {
   switch (studioStore.streamingContext) {
      case 'sales': return SalesOverlay;
      case 'talkshow': return TalkShowOverlay;
      case 'news': return NewsOverlay;
      case 'music': return MusicOverlay;
      case 'gameshow': return GameShowOverlay;
      case 'game_streaming': return GameStreamingOverlay;
      case 'sport': return SportOverlay;
      case 'education': return EducationOverlay;
      case 'commentary': return CommentaryOverlay;
      case 'general': return GeneralOverlay;
      default: return null;
   }
});
...
onUnmounted(() => {
   ...
   import('@/utils/ai/AudienceIntelligenceService').then(({ audienceIntelligenceService }) => {
        audienceIntelligenceService.stop();
   });
});
...
const guestPersonas = computed(() => {
   ...
   // 1. Synthetic Guests (Influencers)
   const influencers = library.map(p => ({
      ...p,
...
// Computed: Is any Influencer connected via LiveChat?
const isGeminiConnected = computed(() => connectedInfluencers.value.length > 0);

// Computed: Is any Influencer speaking?
const isGeminiSpeaking = computed(() => isAnyInfluencerSpeaking.value);

// Computed: Audio level of active Influencer
const geminiAudioLevel = computed(() => {
    ...
    // Return max audio level from all connected Influencers
...
const performingInfluencer = ref<any>(null);
...
const handlePerformanceEnded = () => {
    ...
    if (performingInfluencer.value) {
         ...
         const guestRef = virtualGuestRefs[performingInfluencer.value.uuid];
         if (guestRef) {
             guestRef.setAudioLevelOverride(null);
         }
         performingInfluencer.value = null;
    }
};

// Bridge Music Audio Level to Influencer LipSync
const handlePerformanceAudioLevel = (level: number) => {
    performanceAudioLevel.value = level;
    if (performingInfluencer.value && performanceModalVisible.value) {
        const guestRef = virtualGuestRefs[performingInfluencer.value.uuid];
        if (guestRef) {
...
// Audio Analysis
const { audioLevel: hostLevel, isSpeaking: hostSpeaking, attachToStream: updateAudioStream } = useAudioVisualizer();
...
// LiveChat Manager (Auto-connect Influencers)
...
const {
    ...
    connectedInfluencers: connectedInfluencers,
    isAnyInfluencerSpeaking: isAnyInfluencerSpeaking,
    syncConnections: syncLiveChatConnections,
    disconnectAll: disconnectAllLiveChat,
    setToolCallCallback: setLiveChatToolCallback,
    broadcastToInfluencers: broadcastToInfluencers,
    connectInfluencer: connectLiveChat,
    disconnectInfluencer: disconnectLiveChat
} = useLiveChatManager();
...
// Computed: Is any Influencer connected via LiveChat? (Moved to top)
// Computed: Is any Influencer speaking? (Moved to top)
// Computed: Audio level of active Influencer (Moved to top)
    ...
    try{
        ...
        if (fn === 'change_expression') {
            ...
            try {
                ...
                const res = await influencerStore.generateImage({ prompt: args.prompt });
            ...
            if (performingInfluencer.value) {
                ...
                const personaId = performingInfluencer.value.uuid || performingInfluencer.value.id;
...
const handlePerformSongTool = async (persona: any, args: any) => {
    ...
    try {
        ...
        performingInfluencer.value = persona;
...
const handleMusicSelected = async (musicData: any) => {
    ...
    const guest = syntheticGuestManager.getGuests().find(
        g => g.persona.uuid === selectedInfluencerId.value
    );
    ...
    if (guest) {
        ...
        await influencerStore.updatePerformanceConfig(selectedInfluencerId.value, guest.persona.performanceConfig, true);
        
        toast.success(t('studio.messages.musicSynced', { name: guest.persona.name || 'Influencer' }));
    }
};
...
const handleStartPerformance = (id: string) => {
    ...
    if (guest && guest.persona.performanceConfig?.backgroundMusic) {
        performingInfluencer.value = guest.persona;
...
const handleProductHighlight = (productId: string) => {
    ...
    if (product) {
        studioStore.showcaseProduct(product);
        toast.info(t('studio.messages.productShowcase', { name: product.name }));
    }
};
...
// Wire Audio Director Auto-Ducking
watch(audioLevel, (val) => {
    if (studioStore.godModeEnabled) {
        neuralAudioDirector.processAutoDucking(val);
    }
});
...
const captureCanvasSnapshot = async () => {
    ...
    try {
        ...
        const response = await influencerStore.analyzeVision({ 
            image: imageData,
...
const toggleGodMode = () => {
   ...
   if (studioStore.godModeEnabled) {
      ...
      neuralAudioDirector.setEnabled(true);
   } else {
      toast.info(t('studio.messages.godModeDisabled'));
      studioDirector.setActive(false);
      neuralAudioDirector.setEnabled(false);
   }
};

const directorLog = ref<any[]>([]);

const addDirectorLog = (action: string, reason: string, logs?: any[]) => {
    directorLog.value.unshift({ action, reason, timestamp: Date.now(), agentLogs: logs });
    if (directorLog.value.length > 20) directorLog.value.pop();
    (window as any)._directorLog = directorLog.value;
};

// Sync logs from StudioProducer notes
watch(() => studioProducer.notes.value, (newNotes) => {
    if (newNotes.length > 0) {
        const latest = newNotes[0];
        // Check if this note is already in our log (simple check by timestamp/title)
        const alreadyLogged = directorLog.value.some(l => l.timestamp === latest.timestamp);
        if (!alreadyLogged) {
            addDirectorLog(latest.title, latest.description, (latest as any).logs);
        }
    }
}, { deep: true });
...
watch(() => studioStore.demoMode, (active) => {
    if (active) {
        ...
        demoInterval = setInterval(() => {
            ...
            const mockMsgs = [
                'Wow, the Influencer looks so real!',
      ...
      for (let i = event.resultIndex; i < event.results.length; ++i) {
         if (event.results[i].isFinal) {
            ...
            if (isTranslating.value && targetLang.value !== sourceLang.value) {
               try {
                  const res = await influencerStore.translateText({ 
                     text: rawText, 
                  ...
                  // Bridge to Gemini for Influencer Dubbing
                  if (isGeminiConnected.value) {
                      broadcastToInfluencers(`[Translate & Dub] From ${sourceLang.value} to ${targetLang.value}: ${rawText}`);
                  }
               } catch (e) {
...
watch(() => studioStore.highlightedProduct, async (product) => {
    ...
    // 2. Generate Pitch Script (Storyboard lookup or fallback)
    let script = '';
    const storyboardLine = studioStore.activeScript?.find((s: any) => 
        (s.productId === product._id || s.productId === product.id) && s.type.toLowerCase().includes('showcase')
    );

    if (storyboardLine) {
        script = storyboardLine.text;
    } else {
        const lang = userStore.preferredLanguage === 'vi' ? 'Vietnamese' : 'English';
        script = `[DIRECTIVE: PERFORM THIS SCRIPT NOW DIRECTLY TO THE AUDIENCE]: Các bạn ơi, nhìn xem mình đang có gì nè! Đây là ${product.name}, một sản phẩm cực kỳ xịn sò luôn! Giá chỉ có ${product.price} ${product.currency || 'VNĐ'} thôi. Mọi người nhanh tay nhấn vào link hoặc quét mã QR trên màn hình để sở hữu ngay nhé! 🔥 (Thể hiện tự nhiên bằng tiếng ${lang})`;
    }
    ...
    if (activeAI) {
        ...
        influencerDubber.setDubbing(true, userStore.preferredLanguage === 'vi' ? 'vi-VN' : 'en-US');
        influencerDubber.dubText(script);
        
        // Inform AI agents of the event
        broadcastToInfluencers(`[Product Pitch] Product: ${product.name}. Description: ${product.description}. Script: ${script}`);
    } else {
...
const handleTalkGuest = async ({ id, prompt }: { id: string, prompt: string }) => {
   ...
   // Check if this influencer is connected via LiveChat Manager
   ...
   if (connection && connection.isConnected) {
      ...
      // Fallback to HTTP API for non-LiveChat influencers
      ...
      if (isGeminiConnected.value) {
         ...
         if (decisions.length > 0) {
            ...
            if (conversationRouter.isMultiResponseTrigger(msg.text)) {
                broadcastToInfluencers(`[Chat] ${msg.user}: ${msg.text}`);
            }
         }
      }
...
onMounted(async () => {
   ...
   try {
      // Only initialize camera if NOT in Human Free Mode AND explicitly requested.
      // Per user request, NEVER auto-start the camera on page load.
      console.log("[Studio] Camera auto-start deferred. User must explicitly enable it.");
      camOn.value = false;
      micOn.value = false;
      ...
      if (isGuest.value) {
         ...
         window.addEventListener('guest:approved', (e: any) => {
          ...
          await fetchAccounts();
          const vtData = await influencerStore.fetchInfluencers();
      ...
      console.error("Camera deferred: ", e);
   }
});
...
watch(() => studioStore.humanFreeMode, async (isFree) => {
    if (isFree) {
        toast.info("🎭 Influencer Only Mode Active: Webcam Disabled");
        if (hostStream.value) {
...
// Auto-sync LiveChat connections when influencers join/leave OR when persona data is loaded
...
watch(() => guestPersonas.value, async (newPersonas) => {
    if (newPersonas.length > 0 && Object.keys(studioStore.guestSlotMap).length > 0) {
        // Re-sync when influencer data arrives to catch any missed initial connections
...
// Handle tool calls from Gemini Live influencers
...
// Sync influencer Audio Tracks to AudioMixerService
    ...
    Object.values(connections).forEach((conn: any) => {
        if (conn.isConnected && conn.geminiLive) {
            ...
            if (stream && stream.value) {
                audioMixerService.addTrack(`influencer_${conn.personaId}`, stream.value);
            }
        } else {
...
.context-blur-enter-active, .context-blur-leave-active {
