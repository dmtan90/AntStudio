<template>
  <div class="education-studio h-screen bg-[#080810] text-white flex flex-col font-sans overflow-hidden">
    <div class="studio-viewport relative flex-1 p-2 flex flex-col items-center justify-center">
      <div class="relative w-full h-full overflow-hidden rounded-[2.5rem] border border-white/10 shadow-3xl transition-all duration-700 bg-black/40">
        ...
        <div class="absolute top-8 left-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="h-10 w-10 rounded-xl bg-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
            <svg viewBox="0 0 24 24" class="h-6 w-6 text-white"><path fill="currentColor" d="M12 3L1 9l11 6 9-4.91V17h2V9L12 3z"/><path fill="currentColor" d="M3.88 10.13L12 14.57l8.11-4.44L12 5.69l-8.12 4.44z"/></svg>
          </div>
          <span class="text-xl font-black uppercase tracking-tighter text-white/90">ANT ACADEMY</span>
        </div>
        ...
        <div class="absolute top-8 right-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5 backdrop-blur-xl">
            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-emerald-400 to-green-600 border border-white/20"></div>
            <span class="text-xs font-bold text-white/80">{{ userStore.user?.username || 'Teacher' }}</span>
          </div>
        </div>
        ...
        <div v-if="!isStudioReady" class="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-[#080810]/95 backdrop-blur-3xl">
          <div class="relative h-24 w-24">
            ...
            <div class="absolute inset-4 rounded-full border-b-2 border-green-500 animate-spin-slow"></div>
          </div>
          <h2 class="mt-8 text-2xl font-black uppercase tracking-widest text-white animate-pulse">Initializing Academy Mode</h2>
          <p class="mt-2 text-sm text-white/20 font-medium uppercase tracking-[0.3em]">Synching Neural Knowledge Graph</p>
        </div>
...
// Local State
const viewers = ref(Math.floor(Math.random() * 1200) + 400);
...
onMounted(async () => {
   ...
   // Force Context
   studioStore.streamingContext = 'education';
   
   // Apply Context Preset
   studioStore.applyContextPreset('education');
   ...
   import('@/utils/ai/StudioDirector').then(({ studioDirector }) => {
      studioDirector.applyThematicLayout('education');
   });
...
<style scoped>
.education-studio { background: radial-gradient(circle at 50% 100%, #0a1520 0%, #080810 100%); }
.studio-viewport {
