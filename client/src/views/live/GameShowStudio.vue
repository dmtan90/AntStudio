<template>
  <div class="gameshow-studio h-screen bg-[#080810] text-white flex flex-col font-sans overflow-hidden">
    <div class="studio-viewport relative flex-1 p-2 flex flex-col items-center justify-center">
      <div class="relative w-full h-full overflow-hidden rounded-[2.5rem] border border-white/10 shadow-3xl transition-all duration-700 bg-black/40">
        ...
        <div class="absolute top-8 left-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="h-10 w-10 rounded-xl bg-yellow-600 flex items-center justify-center shadow-lg shadow-yellow-500/20">
            <svg viewBox="0 0 24 24" class="h-6 w-6 text-white"><path fill="currentColor" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
          </div>
          <span class="text-xl font-black uppercase tracking-tighter text-white/90">ANT BRAIN SHOW</span>
        </div>
        ...
        <div class="absolute top-8 right-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5 backdrop-blur-xl">
            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 border border-white/20"></div>
        ...
        <div v-if="!isStudioReady" class="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-[#080810]/95 backdrop-blur-3xl">
          <div class="relative h-24 w-24">
            ...
            <div class="absolute inset-4 rounded-full border-b-2 border-orange-500 animate-spin-slow"></div>
          </div>
          <h2 class="mt-8 text-2xl font-black uppercase tracking-widest text-white animate-pulse">Initializing Gameshow Mode</h2>
          <p class="mt-2 text-sm text-white/20 font-medium uppercase tracking-[0.3em]">Connecting Swarm Intelligence Hub</p>
        </div>
...
// Local State
const viewers = ref(Math.floor(Math.random() * 3000) + 1000);
...
onMounted(async () => {
   ...
   // Force Context
   studioStore.streamingContext = 'gameshow';
   
   // Apply Context Preset
   studioStore.applyContextPreset('gameshow');
   ...
   import('@/utils/ai/StudioDirector').then(({ studioDirector }) => {
      studioDirector.applyThematicLayout('gameshow');
   });
...
<style scoped>
.gameshow-studio { background: radial-gradient(circle at 50% 100%, #15150a 0%, #080810 100%); }
.studio-viewport {
