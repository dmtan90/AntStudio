<template>
  <div class="news-studio h-screen bg-[#080810] text-white flex flex-col font-sans overflow-hidden">
    <div class="studio-viewport relative flex-1 p-2 flex flex-col items-center justify-center">
      <div class="relative w-full h-full overflow-hidden rounded-[2.5rem] border border-white/10 shadow-3xl transition-all duration-700 bg-black/40">
        ...
        <div class="absolute top-8 left-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="h-10 w-10 rounded-xl bg-red-600 flex items-center justify-center shadow-lg shadow-red-500/20">
          ...
          <span class="text-xl font-black uppercase tracking-tighter text-white/90">ANT NEWS NETWORK</span>
        </div>
        ...
        <div class="absolute top-8 right-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5 backdrop-blur-xl">
            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-red-400 to-red-600 border border-white/20"></div>
            <span class="text-xs font-bold text-white/80">{{ userStore.user?.username || 'Anchor' }}</span>
          </div>
        </div>
        ...
        <div v-if="!isStudioReady" class="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-[#080810]/95 backdrop-blur-3xl">
          ...
          <h2 class="mt-8 text-2xl font-black uppercase tracking-widest text-white animate-pulse">Initializing News Mode</h2>
          <p class="mt-2 text-sm text-white/20 font-medium uppercase tracking-[0.3em]">Connecting Global Insight Grid</p>
        </div>
...
onMounted(async () => {
   ...
   // Force Context
   studioStore.streamingContext = 'news';
   
   // Apply Context Preset
   studioStore.applyContextPreset('news');
   ...
   import('@/utils/ai/StudioDirector').then(({ studioDirector }) => {
      studioDirector.applyThematicLayout('news');
   });
...
<style scoped>
.news-studio { background: radial-gradient(circle at 50% 100%, #0a1020 0%, #080810 100%); }
.studio-viewport {
