<template>
  <div class="game-studio h-screen bg-[#06060c] text-white flex flex-col font-sans overflow-hidden">
      ...
      <div class="relative w-full h-full overflow-hidden rounded-[2.5rem] border border-white/10 shadow-3xl transition-all duration-700 bg-black/40">
        ...
        <div class="absolute top-8 left-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="h-10 w-10 rounded-xl bg-indigo-600 flex items-center justify-center shadow-lg shadow-indigo-500/20">
            <svg viewBox="0 0 24 24" class="h-6 w-6 text-white"><path fill="currentColor" d="M21 6H3c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-10 7H8v3H6v-3H3v-2h3V8h2v3h3v2zm4.5 2c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm4-3c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>
          </div>
          <span class="text-xl font-black uppercase tracking-tighter text-white/90">ANTPRO STREAM</span>
        </div>
        ...
        <div class="absolute top-8 right-10 flex items-center gap-4 z-50 pointer-events-auto">
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5 backdrop-blur-xl">
            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-600 border border-white/20"></div>
            <span class="text-xs font-bold text-white/80">{{ userStore.user?.username || 'Gamer' }}</span>
          </div>
        </div>

        <GameStreamingOverlay 
          v-if="isStudioReady"
          :viewers="viewers" 
          :duration="liveTime" 
        ...
        <div v-if="!isStudioReady" class="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-[#080810]/95 backdrop-blur-3xl">
          ...
          <h2 class="mt-8 text-2xl font-black uppercase tracking-widest text-white animate-pulse">Initializing Pro Gamer Mode</h2>
          <p class="mt-2 text-sm text-white/20 font-medium uppercase tracking-[0.3em]">Calibrating Zero-Latency Neural Link</p>
        </div>
...
// Local State
const viewers = ref(Math.floor(Math.random() * 8000) + 3000);
...
onMounted(async () => {
   ...
   // Force Context
   studioStore.streamingContext = 'game_streaming';
   
   // Apply Context Preset
   studioStore.applyContextPreset('game_streaming');
   ...
   import('@/utils/ai/StudioDirector').then(({ studioDirector }) => {
      studioDirector.applyThematicLayout('game_streaming');
   });
...
<style scoped>
.game-studio { background: radial-gradient(circle at 50% 100%, #0a2015 0%, #080810 100%); }
.studio-viewport {
