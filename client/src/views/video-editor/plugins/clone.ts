...
export class CanvasClone {
  ...
  async paste(): Promise<fabric.Object | EditorAudioElement> {
    ...
    const name = (type == "audio") ?
      FabricUtils.elementID((this._clipboard as any).id!.split("_").at(0) || "clone") :
      FabricUtils.elementID((this._clipboard as any).name!.split("_").at(0) || "clone");
    ...
    if (this._clipboard.clipPath) {
      ...
      FabricUtils.bindObjectTransformToParent(clone as any, [clipPath]);
      const handler = () => FabricUtils.updateObjectTransformToParent(clone as any, [{ object: clipPath }]);

      (clone as any).on("moving", handler);
      (clone as any).on("scaling", handler);
      (clone as any).on("rotating", handler);
      (clone as any).set({ clipPath }).setCoords();

      this.canvas.instance.add(clipPath, clone as any);
      this.canvas.instance.setActiveObject(clone as any).requestRenderAll();
      ...
      this.canvas.instance.add(clone as any);
      this.canvas.instance.setActiveObject(clone as any).requestRenderAll();
    } else {
      ...
      await (this.canvas as any).audio?.initialize([clone]);
      clone = (this.canvas as any).audio.get(name)
    }
  ...
  async clone(object?: fabric.Object | EditorAudioElement): Promise<fabric.Object> {
    this.copy(object);
    const cloned: any = await this.paste();
