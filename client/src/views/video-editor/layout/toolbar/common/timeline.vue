...
import { Down as ChevronDown, Timeline as GanttChart, Layers, Send, Magic } from '@icon-park/vue-next';
...
<template>
  <div class="flex items-center gap-3.5 px-1 py-1">
    ...
    <button
      class="cinematic-button !h-9 !px-4 !rounded-xl border-white/5 flex items-center gap-2.5 group transition-all duration-300 shadow-sm"
      :class="[
        editor.sidebarRight === 'transition' ? 'bg-white/15 border-white/20 text-white shadow-lg' : 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white'
      ]" @click="editor.setActiveSidebarRight(editor.sidebarRight === 'transition' ? null : 'transition')">
      <Magic :size="16" class="text-white/40 group-hover:text-white transition-colors" />
      <span class="text-[10px] font-bold uppercase tracking-widest">Transitions</span>
    </button>
    ...
    <el-popover placement="bottom-end" trigger="click" width="280"
      ...
      <template #reference>
        <button
          class="cinematic-button !h-9 !px-4 !rounded-xl border-white/5 bg-white/5 flex items-center gap-2.5 group transition-all duration-300 hover:bg-white/10 hover:border-white/10 text-white/50 hover:text-white shadow-sm">
          ...
          <div class="flex flex-col gap-3">
            <div class="flex justify-between items-center mb-1">
              ...
              <span class="text-[10px] font-bold font-mono text-white/60 bg-white/5 px-1.5 rounded">{{
                durationInSecond.toFixed(2) }}s</span>
            </div>
            <SliderInput :model-value="durationInSecond" :min="0.1" :max="timeline.duration / 1000" :step="0.05"
              @update:model-value="(value) => durationInSecond = value" />
          </div>
          <div class="flex flex-col gap-3">
            <div class="flex justify-between items-center mb-1">
              <Label class="text-[10px] font-bold text-white/20 uppercase tracking-widest">Offset</Label>
              <span class="text-[10px] font-bold font-mono text-white/60 bg-white/5 px-1.5 rounded">{{
                offsetInSecond.toFixed(2) }}s</span>
            </div>
            <SliderInput :model-value="offsetInSecond" :min="0" :max="timeline.duration / 1000" :step="0.05"
              @update:model-value="(value) => offsetInSecond = value" />
          </div>
        </div>
      </div>
    </el-popover>
