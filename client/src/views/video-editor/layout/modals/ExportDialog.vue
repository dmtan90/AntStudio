...
const qualityPresets = [
    {
        id: 'draft',
        name: 'Draft',
        description: 'Fast export for previews',
        icon: '⚡',
        settings: {
            resolution: '720p',
            fps: 24,
    ...
    },
    {
        id: 'high',
        name: 'High',
        description: 'High quality for sharing',
        icon: '💎',
        settings: {
            resolution: '1080p',
            fps: 60,
...
const bitrateOptions = [
    { label: 'Low (5 Mbps)', value: 'low' },
    { label: 'Medium (10 Mbps)', value: 'medium' },
    { label: 'High (20 Mbps)', value: 'high' },
    { label: 'Ultra (40 Mbps)', value: 'ultra' }
];

const codecOptions = [
    { label: 'H.264 (Compatible)', value: 'h264' },
    { label: 'H.265 (Smaller file)', value: 'h265' }
];

const currentPreset = computed(() => {
    return qualityPresets.find(p => p.id === selectedPreset.value);
});
...
watch(selectedPreset, (preset) => {
    const presetData = qualityPresets.find(p => p.id === preset);
    if (presetData) {
    ...
    <el-dialog v-model="showDialog" :width="700" :close-on-click-modal="false" :close-on-press-escape="!isExporting"
        ...
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-black text-white uppercase tracking-wider">Export Video</h2>
                    <p class="text-[10px] text-white/40 font-bold uppercase">
                        {{ isExporting ? 'Exporting...' : 'Choose quality preset' }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <div class="w-full space-y-2">
                    <div class="flex justify-between text-xs">
                        <span class="text-white/60 font-bold uppercase">Time Remaining</span>
                        <span class="text-orange-400 font-mono">{{ estimatedTime }}</span>
                    </div>
                    <div class="flex justify-between text-xs">
                        <span class="text-white/60 font-bold uppercase">Estimated Size</span>
        ...
        <div v-else class="space-y-6">
            ...
            <div class="p-4 rounded-xl bg-white/5 border border-white/10 space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-white/60 uppercase">Advanced Settings</span>
                    <span class="text-[10px] text-orange-400 font-mono">{{ estimatedFileSize }}</span>
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1">
                        <label class="text-[10px] text-white/40 font-bold uppercase">Resolution</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] text-white/40 font-bold uppercase">Frame Rate</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] text-white/40 font-bold uppercase">Bitrate</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] text-white/40 font-bold uppercase">Codec</label>
