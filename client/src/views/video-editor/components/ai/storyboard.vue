...
import { generateStoryboard } from 'video-editor/api/ai';
...
const onGenerateStoryboard = async () => {
    ...
    try {
        ...
        const session = await generateStoryboard({ 
        ...
        if (!session || !session.scene) {
            throw new Error("Invalid storyboard generated");
        }

        progress.value = 50;
        status.value = 'assembling';

        // Step 2: Use the existing Prompt plugin logic to build the scenes
        // The prompter.createSceneFromPromptSession already handles:
        // - Clearing canvas
        // - Setting artboard dimensions
        // - Adding video, speech, audio, and subtitles for each scene
