...
/**
 * Service to handle real-time Influencer Dubbing (Voice Cloning + Translation).
 * Translates Host's voice into target languages while maintaining their unique vocal tone.
 */
export class InfluencerDubber {
    ...
    public initialize(voiceId: string) {
        ...
        console.log(`[InfluencerDubber] Initialized with VoiceID: ${voiceId}`);
    }

    /**
     * Processes a chunk of text (from STT) and generates Influencer audio.
    ...
    public async dubText(text: string, lang?: string): Promise<string | null> {
        ...
        try {
            console.log(`[InfluencerDubber] Dubbing text to ${lang || this.targetLanguage}: "${text.substring(0, 30)}..."`);
            ...
            console.error("[InfluencerDubber] Dubbing failed:", error);
            return null;
        }
    }
    ...
    private queueAudio(url: string) {
        ...
        audio.onerror = (e) => {
            console.error("[InfluencerDubber] Audio playback error:", e);
            this.isPlaying = false;
            this.playNext();
        };
    ...
    private playNext() {
        ...
        if (nextAudio) {
            ...
            nextAudio.play().catch(e => {
                console.error("[InfluencerDubber] Play failed:", e);
                this.isPlaying = false;
                this.playNext();
            });
        }
    ...
    public setDubbing(active: boolean, targetLang?: string) {
        ...
        console.log(`[InfluencerDubber] Influencer Dubbing: ${active ? 'ACTIVE' : 'OFF'} (Target: ${this.targetLanguage})`);
    }
...
export const influencerDubber = new InfluencerDubber();
