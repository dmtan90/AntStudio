...
/**
 * Service to handle real-time Neural Dubbing (Voice Cloning + Translation).
 * Translates Host's voice into target languages while maintaining their unique vocal tone.
 */
export class NeuralDubber {
    ...
    public initialize(voiceId: string) {
        ...
        console.log(`[NeuralDubber] Initialized with VoiceID: ${voiceId}`);
    }

    /**
     * Processes a chunk of text (from STT) and generates neural audio.
    ...
    public async dubText(text: string, lang?: string): Promise<string | null> {
        ...
        try {
            console.log(`[NeuralDubber] Dubbing text to ${lang || this.targetLanguage}: "${text.substring(0, 30)}..."`);
            ...
            console.error("[NeuralDubber] Dubbing failed:", error);
            return null;
        }
    }
    ...
    private queueAudio(url: string) {
        ...
        audio.onerror = (e) => {
            console.error("[NeuralDubber] Audio playback error:", e);
            this.isPlaying = false;
            this.playNext();
        };
    ...
    private playNext() {
        ...
        if (nextAudio) {
            ...
            nextAudio.play().catch(e => {
                console.error("[NeuralDubber] Play failed:", e);
                this.isPlaying = false;
                this.playNext();
            });
        }
    ...
    public setDubbing(active: boolean, targetLang?: string) {
        ...
        console.log(`[NeuralDubber] Neural Dubbing: ${active ? 'ACTIVE' : 'OFF'} (Target: ${this.targetLanguage})`);
    }
...
export const neuralDubber = new NeuralDubber();
