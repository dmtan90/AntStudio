...
function init3DGuest(id: string, glbUrl: string, textureUrl: string) {
    if (threeGuests.has(id)) return;

    console.log(`[Worker] Initializing 3D Guest: ${id}, GLB: ${glbUrl}`);
...
let visualSettings: any = {
    beauty: { smoothing: 0, brightness: 0.5, sharpen: 0, denoise: 0 },
...
void main() {
    ...
    // Map [0,1] to clip space [-1,1], flipping Y so 0 is top
...
self.onmessage = async (e: MessageEvent) => {
    ...
    switch (type) {
        ...
        case 'update-mask':
            ...
            if (payload.maskData) {
                updateMaskTexture(payload.maskData);
                sceneDirty = true;
            }
            break;
        ...
        case 'update-overlay':
            ...
            break;
        case 'add-3d-guest':
            init3DGuest(payload.id, payload.glbUrl, payload.textureUrl);
            break;
        ...
        case 'update-3d-thinking':
            ...
            if (g) g.isThinking = payload.isThinking;
            break;
    }
};
...
function resizeIntermediateTextures() {
    ...
    if (!maskTexture) {
        ...
        const maskData = new Uint8Array(width * height * 4).fill(0);
...
function updateMaskTexture(maskData: ImageData) {
    if (!gl) return;
    if (!maskTexture) maskTexture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, maskTexture!);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, maskData);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
}
...
function renderLoop(time: number = 0) {
    ...
    threeGuests.forEach((guest, id) => {
        ...
        const texture = WebGLUtils.createTexture(gl!);
        if (texture) {
            gl!.bindTexture(gl!.TEXTURE_2D, texture);
...
function drawRegion(region: any) {
    ...
    // Apply effects only to host
    const applyEffects = id === 'host';
    let processedTexture = sourceTexture;

    if (applyEffects && (visualSettings.beauty.smoothing > 0.01 ||
        visualSettings.beauty.brightness !== 1.0 || // Brightness 1.0 is neutral
        visualSettings.beauty.sharpen > 0.01 ||
        visualSettings.beauty.denoise > 0.01 ||
        visualSettings.background.mode !== 'none')) {
        processedTexture = applyVisualEffects(sourceTexture, meta.width, meta.height);
    }

    // Composite to canvas
    renderToCanvas(processedTexture, region, meta);
}

function applyVisualEffects(inputTexture: WebGLTexture, sourceWidth: number, sourceHeight: number): WebGLTexture {
...
function renderToCanvas(texture: WebGLTexture, region: any, meta: { width: number, height: number }) {
    ...
    if (uFlipHorizontalLoc) gl.uniform1i(uFlipHorizontalLoc, region.source === 'host' ? 1 : 0);
    if (uFlipVerticalLoc) gl.uniform1i(uFlipVerticalLoc, 0); // Disabled flip
