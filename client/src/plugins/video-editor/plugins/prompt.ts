...
import { createMap } from "video-editor/lib/utils";
  ...
  async createSceneFromPromptSession(session: PromptSession) {
    try {
      ...
      for (const scene of session.scene) {
        if (scene.video) {
          const isVideo = scene.video.url.toLowerCase().match(/\.(mp4|webm|mov|m4v)$/) || scene.video.url.toLowerCase().includes('video');
          const options = { meta: { duration: scene.duration * 1000, offset } };
          
          let asset: any;
          if (isVideo) {
            asset = await this.canvas.onAddVideoFromSource(scene.video.url, options, true, false);
          } else {
            asset = await this.canvas.onAddImageFromSource(scene.video.url, options as any, true, false);
          }

          if (asset) {
            const scaleX = this.canvas.artboard.width! / asset.width!;
            const scaleY = this.canvas.artboard.height! / asset.height!;
            asset.scale(Math.max(scaleX, scaleY));
            asset.setPositionByOrigin(this.canvas.artboard.getCenterPoint(), "center", "center");
          }
        }
