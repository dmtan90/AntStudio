...
import type { Dimension } from "video-editor/types/editor";
...
export class Canvas {
    ...
    return createPromise<fabric.Image>((resolve, reject) => {
      fabric.Image.fromURL(
        ...
        (image) => {
          ...
          FabricUtils.initializeMetaProperties(image, options?.meta);
          FabricUtils.initializeAnimationProperties(image, options?.anim);
          (image as any).originalSrc = source;
    ...
    return createPromise<any>((resolve, reject) => {
      (fabric as any).Video.fromURL(
        ...
        (video: any) => {
          ...
          (video as any).originalSrc = source;
  ...
  onAlignActiveObject(type: string) {
    let _type = type;
    if (type === "vcenter") _type = "middle";
    if (type === "hcenter") _type = "center";
    this.alignment.alignActiveObjecToPage(_type as any);
  }

  onDistributeActiveObjects(type: "horizontal" | "vertical") {
    this.alignment.distributeObjects(type);
  }

  onBringForward() {
    this.alignment.changeActiveObjectLayer("up");
  }

  onSendBackward() {
    this.alignment.changeActiveObjectLayer("down");
  }

  onBringToFront() {
    this.alignment.changeActiveObjectLayer("top");
  }

  onSendToBack() {
    this.alignment.changeActiveObjectLayer("bottom");
  }

  onChangeVisualizerProperties(object: any, props: any) {
    if (!object || !props) return;
    
    // Handle visibility if included in props
    if (props.visible !== undefined) {
      object.set("visible", props.visible);
    }
    
    // Handle visualizer mode (mapping 'visualizerMode' to 'visualType')
    if (props.visualizerMode !== undefined) {
      object.set("visualType", props.visualizerMode);
    }
    
    // All other properties are considered visualizer-specific properties (visualProps)
    const { visible, visualizerMode, ...visualProps } = props;
    if (Object.keys(visualProps).length > 0) {
      // If object has a visualProps property, update it
      // Based on fabric/audio.ts, FabricAudio has it
      object.set("visualProps", visualProps);
    }
