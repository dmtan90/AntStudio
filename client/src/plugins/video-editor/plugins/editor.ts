...
import { 
  ...
  ExportProgress,
  type Dimension
} from "video-editor/types/editor";

export { ExportProgress, type ExportMode, type EditorMode, type EditorStatus, type EditorProgress, type Dimension };
import { Adapter } from "./adapter";
import i18n from "@/i18n";
import { useProjectStore } from "@/stores/project";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const t = (key: string, params?: Record<string, string>): string => (i18n.global as any).t(key, params);

// Global FFmpeg instance and loading state
...
export const loadFFmpeg = async () => {
  ...
  ffmpegLoadingPromise = (async () => {
    try {
      // Use local models path for faster loading and offline support
      // Make sure to download these files to /public/models/ffmpeg/
      const baseURL = '/models/ffmpeg';
      await ffmpeg.load({
        coreURL: await toBlobURL(`${baseURL}/ffmpeg-core.js`, "text/javascript"),
        wasmURL: await toBlobURL(`${baseURL}/ffmpeg-core.wasm`, "application/wasm"),
      });
      console.log("[FFmpeg] Loaded successfully from local source");
    } catch (error) {
...
export class Editor {
  ...
  constructor() {
    ...
    this.mode = "creator";
    this.name = t('studio.videoEditor.messages.untitledTemplate');
    ...
    try {
      ...
      try{
        if(this.id && upload){
          ...
          toast.success(t('studio.videoEditor.export.success'));
        ...
        toast.error(t('studio.videoEditor.export.uploadFailed'));
      }
  ...
  async exportProjectToStream(fileHandle: FileSystemFileHandle, progress?: (p: number, status: string) => void): Promise<void> {
    ...
    try {
      if (progress) progress(0, t('studio.videoEditor.export.startingStream'));
      ...
      await this.recorder.mediaBunnyCompile(frameGenerator(this.pages, this.recorder, this.fps) as any, {
        ...
        progress: (p) => {
          if (progress) progress(p.progress, t('studio.videoEditor.export.exportingProgress', { percent: String(Math.round(p.progress * 100)) }));
        }
      });

      if (progress) progress(1, t('studio.videoEditor.export.exportComplete'));
    } catch (e) {
  ...
  async getAssemblyProject(progress?: (p: number, status: string) => void): Promise<any> {
    ...
    for (let i = 0; i < this.pages.length; i++) {
      ...
      if (progress) progress((i / this.pages.length), t('studio.videoEditor.export.renderingScene', { current: String(i + 1), total: String(this.pages.length) }));
      ...
      const frames = await this.recorder.capturePage(i, this.fps, {
        progress: (p) => {
          if (progress) {
            ...
            progress(totalP, t('studio.videoEditor.export.capturingScene', { current: String(i + 1), percent: String(Math.round(p.progress * 100)) }));
          }
        }
      });
    ...
    if (progress) progress(1, t('studio.videoEditor.export.readyForAssembly'));
  ...
  destroy() {
    console.log("[Editor] Destroying editor instance...");
    this.pages.forEach(page => page.destroy());
    this.pages = [];
    this.recorder.stop();
    // Add any other specific cleanup for adapter, prompter if they have destroy methods
    if ((this.adapter as any).destroy) (this.adapter as any).destroy();
    if ((this.prompter as any).destroy) (this.prompter as any).destroy();
    
    this.onModified = undefined;
    this.onSaveRequested = undefined;
    this.onStatusChange = undefined;
    this.onTick = undefined;
    this.onThumbnailUpdated = undefined;
    
    if (this.controller) {
      this.controller.abort({ message: "Editor destroyed" });
    }
  }
}
