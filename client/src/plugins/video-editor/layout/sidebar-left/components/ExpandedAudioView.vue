...
const onAddAudio = async (audio: any) => {
  try {
    ...
    const promise = (editor.canvas as any).onAddAudioFromSource(resolvedSource).then(element => {
      (editor.canvas as any).audio.add(resolvedSource, audio.name, (element as any).name, audio._id);
    });
...
<template>
  <template v-if="match === 'sounds'">
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <span v-if="!loading"
        class="text-[10px] font-bold text-white/20 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 leading-none uppercase tracking-widest text-center">{{ t('videoEditor.audio.noAudiosFound') }}</span>
    </div>
  </template>
  ...
  <template v-if="match === 'musics'">
    ...
    <div v-else class="flex flex-col gap-4">
      <el-skeleton v-for="(_, index) in 6" :key="index" animated class="h-16 w-full rounded-xl !bg-white/5" />
      <span v-if="!loading"
        class="text-[10px] font-bold text-white/20 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 leading-none uppercase tracking-widest text-center">{{ t('videoEditor.audio.noMusicFound') }}</span>
    </div>
  </template>
  ...
  <template v-if="match === 'userAudios'">
    <div class="mb-4">
      <button @click="showVoiceLab = true"
        ...
        <span class="text-xs font-black uppercase tracking-widest text-white">{{ t('videoEditor.audio.voiceLab') }}</span>
      </button>
    </div>
