...
const options = computed(() => [
  {
    value: 'cloud',
    label: t('videoEditor.template.public')
  },
  {
    value: 'local',
    label: t('videoEditor.template.private')
  }
]);
...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel">
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5 shrink-0">
      <h2 class="font-bold text-sm tracking-wider uppercase text-white/90">{{ t('videoEditor.template.title') }}</h2>
      <button
    ...
    <section class="flex-1 overflow-y-auto custom-scrollbar sidebar-container">
      <template v-if="editor.mode === 'creator'">
        <div class="px-5 pt-4 pb-4">
          <el-input :placeholder="t('videoEditor.template.searchPlaceholder')" class="cinematic-input">
            <template #prefix>
      ...
      <template v-else>
        <div class="p-10 flex flex-col items-center justify-center text-center gap-2">
          ...
          <span class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.template.unknownMode') }}</span>
        </div>
      </template>
    </section>
  </div>
