...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel">
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5">
      <h2 class="font-bold text-sm tracking-wider uppercase text-white/90">{{ t('videoEditor.scene.title') }}</h2>
      <div class="flex items-center gap-2 ml-auto">
        <button
          ...
          <span class="text-[10px] font-bold uppercase tracking-wider">{{ t('videoEditor.scene.add') }}</span>
        </button>
        ...
        <div v-for="(_, index) in editor.pages" :key="_.id"
          ...
          <div
            ...
            <div
              ...
              <span class="text-[10px] font-bold text-white uppercase tracking-widest">{{ t('videoEditor.scene.edit') }}</span>
            </div>
          </div>
            ...
            <template #dropdown>
              <el-dropdown-menu class="min-w-20">
                <el-dropdown-item command="up" :icon="UpSquare" v-if="page > 0 && pages.length > 1">
                  <span class="">{{ t('videoEditor.scene.moveUp') }}</span>
                </el-dropdown-item>
                <el-dropdown-item command="down" :icon="DownSquare" v-if="page < pages.length - 1 && pages.length > 1">
                  <span class="">{{ t('videoEditor.scene.moveDown') }}</span>
                </el-dropdown-item>
                <el-dropdown-item command="copy" :icon="Copy">
                  <span class="">{{ t('videoEditor.scene.duplicate') }}</span>
                </el-dropdown-item>
                <el-dropdown-item command="delete" :icon="Delete">
                  <span class="">{{ t('videoEditor.scene.delete') }}</span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
          ...
          <div
            ...
            <span class="text-[10px] font-bold text-white/90 uppercase tracking-widest">{{ t('videoEditor.scene.sceneIndex', { index: index + 1 }) }}</span>
          </div>
