...
const handleUpload = (options: any) => {
  ...
  toast.promise(promise, {
    loading: t('videoEditor.audio.uploadLoading'),
    success: t('videoEditor.audio.uploadSuccess'),
    error: t('videoEditor.audio.uploadError'),
  });
  return promise;
};
...
const onAddAudio = async (audio: any) => {
  try {
    ...
    toast.error(t('videoEditor.audio.loadError'));
  }
};
...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel">
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5 shrink-0">
      <h2 class="font-bold text-sm tracking-wider uppercase text-white/90">{{ t('videoEditor.audio.title') }}</h2>
      <button
    ...
    <div class="px-5 pt-4 pb-2" v-if="expanded">
      <div class="flex gap-2 mb-3">
        ...
        <el-input v-model="query" :placeholder="t('videoEditor.audio.searchPlaceholder')" class="cinematic-input flex-1"
        ...
        <div class="px-5 flex flex-col gap-6 pt-2">
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.audio.uploads') }}</h4>
              <div class="flex items-center gap-2">
                <el-upload :show-file-list="false" :http-request="handleUpload" accept="audio/*">
                  <button
                    ...
                    <span>{{ t('videoEditor.audio.upload') }}</span>
                  </button>
                </el-upload>
            ...
            <div class="flex gap-3 items-center overflow-x-auto scrollbar-hidden relative pb-2 fade-mask-r">
              ...
              <template v-else>
                <div
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-widest">{{ t('videoEditor.audio.noUploads') }}</span>
                </div>
              </template>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.audio.music') }}</h4>
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.audio.soundFx') }}</h4>
              <button class="text-[10px] font-bold text-brand-primary hover:text-white transition-colors"
