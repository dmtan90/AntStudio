<template>
  <div class="cloud-drive-browser p-4 h-full flex flex-col">
    <div class="header mb-6 flex justify-between items-center shrink-0">
      <div>
        <h3 class="text-white font-black uppercase text-xs tracking-[0.2em]">{{ t('videoEditor.cloud.title') }}</h3>
        <p class="text-[10px] text-gray-500 uppercase font-bold tracking-widest mt-1">{{ t('videoEditor.cloud.subtitle') }}</p>
      </div>
    ...
    <div class="search-bar mb-6 relative group">
      ...
      <input 
        v-model="searchQuery" 
        :placeholder="t('videoEditor.cloud.searchPlaceholder')" 
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar sidebar-container pr-2 -mr-2">
      ...
      <div v-else-if="filteredFiles.length === 0" class="flex flex-col items-center justify-center py-20 opacity-30">
        ...
        <p class="text-[10px] uppercase font-black tracking-widest">{{ t('videoEditor.cloud.noAssets') }}</p>
      </div>
    ...
    <div class="mt-4 p-3 bg-blue-500/5 border border-blue-500/10 rounded-2xl flex items-start gap-3">
       ...
       <p class="text-[9px] text-blue-400/70 font-bold uppercase leading-relaxed">
         {{ t('videoEditor.cloud.corsWarning') }}
       </p>
    </div>
  </div>
</template>
...
const importAsset = async (file: any) => {
  try {
    ...
    if (!type) {
      toast.error(t('videoEditor.cloud.unsupportedFormat'));
      return;
    }
    ...
    toast.success(t('videoEditor.cloud.syncSuccess'));
  } catch (error) {
    toast.error(t('videoEditor.cloud.importError'));
  }
};
