...
const onAddGiphyVideo = async (source: string, thumbnail: string) => {
  try {
    ...
    if (((editor.canvas as any).replacer.active as any)?.type === "gif") {
      ...
      toast.promise(promise, { loading: t('videoEditor.element.replaceGifLoading'), success: t('videoEditor.element.replaceGifSuccess'), error: t('videoEditor.element.replaceGifError') });
    } else if (!thumbnail || !isImageLoaded(thumbnail as any)) {
      const promise = (editor.canvas as any).onAddGifFromSource(resolvedSource);
      toast.promise(promise, { loading: t('videoEditor.element.addGifLoading'), success: t('videoEditor.element.addGifSuccess'), error: t('videoEditor.element.addGifError') });
    } else {
      toast.promise((editor.canvas as any).onAddGifFromThumbnail(resolvedSource, thumbnail), { error: t('videoEditor.element.addGifError') });
    }
  } catch (error) {
    console.error('Failed to resolve gif source:', error);
    toast.error(t('videoEditor.element.loadGifError'));
  }
};
...
const onAddEmoji = async (source: string, thumbnail: string) => {
  try {
    ...
    if (((editor.canvas as any).replacer.active as any)?.type === "gif") {
      const promise = (editor.canvas as any).replacer.replace(resolvedSource, true);
      toast.promise(promise, { loading: t('videoEditor.element.replaceEmojiLoading'), success: t('videoEditor.element.replaceEmojiSuccess'), error: t('videoEditor.element.replaceEmojiError') });
    } else if (!thumbnail || !isImageLoaded(thumbnail as any)) {
      const promise = (editor.canvas as any).onAddGifFromSource(resolvedSource);
      toast.promise(promise, { loading: t('videoEditor.element.addEmojiLoading'), success: t('videoEditor.element.addEmojiSuccess'), error: t('videoEditor.element.addEmojiError') });
    } else {
      const promise = (editor.canvas as any).onAddGifFromThumbnail(resolvedSource, thumbnail);
      toast.promise(promise, { error: t('videoEditor.element.addEmojiError') });
    }
  } catch (error) {
    console.error('Failed to resolve emoji source:', error);
    toast.error(t('videoEditor.element.loadEmojiError'));
  }
};
...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel">
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5 shrink-0">
      <h2 class="font-bold text-sm tracking-wider uppercase text-white/90">{{ t('videoEditor.element.title') }}</h2>
    ...
    <div class="px-5 pt-4 pb-2" v-if="expanded">
      <div class="flex gap-2 mb-3">
         ...
         <el-input v-model="query" :placeholder="t('videoEditor.element.searchPlaceholder')" class="cinematic-input flex-1" @change="handleResetData">
            <template #prefix>
        ...
        <div class="px-5 flex flex-col gap-6 pt-2">
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.basicShapes') }}</h4>
              <button class="text-[10px] font-bold text-brand-primary hover:text-white transition-colors" @click="expanded = 'basic'">
                {{ t('videoEditor.element.seeAll') }}
              </button>
            </div>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.abstractShapes') }}</h4>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.frames') }}</h4>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
             <div class="flex items-center justify-between">
               <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.lines') }}</h4>
             </div>
             <div class="flex gap-3 items-center overflow-x-auto scrollbar-hidden relative pb-2 fade-mask-r">
               <button
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.gifs') }}</h4>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.stickers') }}</h4>
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.element.emoji') }}</h4>
