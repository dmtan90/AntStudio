...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.template.localTemplates') }}</h4>
      <el-upload :show-file-list="false" :http-request="handleLoadJSON" accept="application/json">
        <button
          ...
          <span>{{ t('videoEditor.template.loadJson') }}</span>
        </button>
      </el-upload>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 items-center relative">
      ...
      <template v-else>
        ...
        <div class="absolute inset-0 flex items-center justify-center">
          <span class="text-[10px] font-bold text-white/20 uppercase tracking-widest">{{ t('videoEditor.template.noLocalTemplates') }}</span>
        </div>
      </template>
    </div>
  </div>
