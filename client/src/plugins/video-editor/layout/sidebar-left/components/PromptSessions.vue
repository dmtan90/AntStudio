...
const handleLoadPromptSession = async (session: PromptSession) => {
  ...
  toast.promise(promise, { loading: t('videoEditor.prompt.loadingSession'), success: t('videoEditor.prompt.sessionLoaded'), error: t('videoEditor.prompt.loadSessionError') });
};
...
<template>
  <section class="sidebar-container pt-4 pb-10 px-5">
    <div class="mb-6">
      <el-input :placeholder="t('videoEditor.prompt.searchPlaceholder')" class="cinematic-input" >
        <template #prefix>
    ...
    <div class="mb-8">
      ...
      >
        ...
        <span>{{ t('videoEditor.prompt.newSession') }}</span>
      </button>
    </div>

    <div class="flex flex-col gap-4">
      <div class="flex items-center justify-between">
        <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.prompt.recentSessions') }}</h4>
        <button class="text-[10px] font-bold text-brand-primary hover:text-white transition-colors">
          {{ t('videoEditor.prompt.seeAll') }}
        </button>
      </div>
        ...
        >
          ...
          <div class="flex items-center gap-4 mt-1">
             <div class="flex items-center gap-1.5">
                <span class="text-[9px] font-bold text-white/20 uppercase tracking-widest">{{ t('videoEditor.prompt.duration') }}</span>
