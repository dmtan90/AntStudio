...
const handleUpload = (options: any) => {
  ...
  toast.promise(promise, {
    loading: t('videoEditor.image.uploadLoading'),
    success: t('videoEditor.image.uploadSuccess'),
    error: t('videoEditor.image.uploadError'),
  });
  return promise;
};
...
const onAddImage = async (source: string, thumbnail: string, preload: boolean) => {
  try {
    ...
    if (((editor.canvas as any).replacer.active as any)?.type === "image") {
      ...
      toast.promise(promise, { loading: t('videoEditor.image.replaceLoading'), success: t('videoEditor.image.replaceSuccess'), error: t('videoEditor.image.replaceError') });
      ...
      toast.promise(promise, { loading: t('videoEditor.image.addLoading'), success: t('videoEditor.image.addSuccess'), error: t('videoEditor.image.addError') });
    } else {
      const promise = (editor.canvas as any).onAddImageFromThumbnail(resolvedSource, thumbnail);
      toast.promise(promise, { error: t('videoEditor.image.addError') });
    }
  } catch (error) {
...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel">
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5 shrink-0">
      <h2 class="font-bold text-sm tracking-wider uppercase text-white/90">{{ t('videoEditor.image.title') }}</h2>
      <button
    ...
    <div class="px-5 pt-4 pb-2" v-if="expanded">
      <div class="flex gap-2 mb-3">
        ...
        <el-input v-model="query" :placeholder="t('videoEditor.image.searchPlaceholder')" class="cinematic-input flex-1"
        ...
        <div class="px-5 flex flex-col gap-6 pt-2">
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.image.uploads') }}</h4>
            ...
            <div class="flex gap-3 items-center overflow-x-auto scrollbar-hidden relative pb-2 fade-mask-r">
              ...
              <template v-else>
                <div
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-widest">{{ t('videoEditor.image.noUploads') }}</span>
                </div>
              </template>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-4 border-b border-white/5 pb-6">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.image.pixels') }}</h4>
              <button class="text-[10px] font-bold text-brand-primary hover:text-white transition-colors"
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.image.unsplash') }}</h4>
              <button class="text-[10px] font-bold text-brand-primary hover:text-white transition-colors"
