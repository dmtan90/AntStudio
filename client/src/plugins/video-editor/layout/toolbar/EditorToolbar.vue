...
import ActiveSelectionToolbar from './components/activeSelection.vue';
...
const toolbarComponentMap: Record<string, any> = {
  ...
  crop: CropToolbar,
  trim: TrimToolbar,
  activeSelection: ActiveSelectionToolbar,
  group: ActiveSelectionToolbar,
};
...
<template>
  <template v-if="!isTablet">
    <transition enter-active-class="transition duration-500 ease-out"
      ...
      <aside v-if="Toolbar"
        :class="cn('h-[60px] absolute top-20 left-1/2 -translate-x-1/2 bg-[#0d0d0d]/80 backdrop-blur-3xl border border-white/10 flex items-center z-20 gap-3 rounded-2xl px-6 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] group transition-all duration-500 hover:border-white/20 hover:bg-[#0a0a0a]/90')">
        <!-- Inner Glow -->
        <div class="absolute inset-0 rounded-2xl bg-gradient-to-b from-white/5 to-transparent pointer-events-none">
        </div>
  ...
  <template v-else>
    <div v-if="Toolbar"
      class="h-14 border-b border-white/5 bg-[#0a0a0a]/95 backdrop-blur-xl px-4 shrink-0 overflow-x-auto custom-scrollbar flex items-center">
