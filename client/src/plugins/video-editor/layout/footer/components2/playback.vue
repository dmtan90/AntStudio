...
const duration = computed({
  get() {
    return editor.totalDuration / 1000;
  },
  set(value) {
    // Current logic sets duration for ONE timeline (active scene). 
    // For global duration, we might want to scale or forbid editing here.
    // For now, let's keep it read-only or just log, as changing total duration implies changing individual scene durations.
    console.warn("Global duration setting not yet implemented from playback controls");
  }
});
...
<template>
  <div
    class="h-14 sm:h-16 px-6 flex items-center gap-6 justify-between border-b border-white/5 shrink-0 overflow-x-auto custom-scrollbar flex-nowrap bg-[#0a0a0a]/50">
    <div class="flex items-center gap-5">
      <div class="relative group">
        <div
          class="absolute inset-0 bg-brand-primary/20 blur-xl rounded-full scale-150 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
        </div>
        <button
          ...
          :disabled="animations.previewing" @click="handleTimelineToggle">
      ...
      <div
        class="flex items-center gap-2 font-mono tabular-nums select-none min-w-[140px] bg-white/5 px-3 py-1.5 rounded-xl border border-white/5 shadow-inner">
        <span class="text-[14px] font-black text-white/90 drop-shadow-sm">{{ formatMediaDuration(timeline.seek, false)
          }}</span>
        <span class="text-[10px] font-bold text-white/20 mx-0.5">/</span>
        <span class="text-[11px] font-bold text-white/40">{{ formatMediaDuration(duration * 1000, false) }}</span>
      </div>
      ...
      <div class="flex items-center gap-1.5" v-if="active">
        <template v-if="splitEnabled">
          <div class="flex items-center bg-white/5 rounded-xl border border-white/5 p-1 gap-1">
            <el-tooltip :content="t('videoEditor.playback.splitClip')" placement="top" popper-class="cinematic-tooltip">
              <button
                class="h-8 px-2.5 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/50 hover:text-white transition-all active:scale-95"
                @click="onSplitItem()">
                <Split :size="16" />
              </button>
            </el-tooltip>
            <el-tooltip :content="t('videoEditor.playback.splitKeepLeft')" placement="top" popper-class="cinematic-tooltip">
              <button
                class="h-8 px-2.5 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/50 hover:text-white transition-all active:scale-95"
                @click="onSplitItem()">
                <KeepLeft :size="16" />
            ...
            <el-tooltip :content="t('videoEditor.playback.splitKeepRight')" placement="top" popper-class="cinematic-tooltip">
              <button
                class="h-8 px-2.5 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/50 hover:text-white transition-all active:scale-95"
                @click="onSplitItem()">
                <KeepRight :size="16" />
        ...
        <el-tooltip :content="t('videoEditor.playback.duplicateClip')" placement="top" v-if="!active?.meta?.thumbnail"
          popper-class="cinematic-tooltip">
          <button
            class="h-9 w-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 text-white/50 hover:text-white transition-all active:scale-95 shadow-sm"
            @click="onCloneItem()">
            <Copy :size="16" />
          </button>
        </el-tooltip>

        <el-tooltip :content="t('videoEditor.playback.deleteElement')" placement="top" popper-class="cinematic-tooltip">
          <button
            class="h-9 w-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/5 hover:bg-red-500/10 hover:border-red-500/20 text-white/40 hover:text-red-500 transition-all active:scale-95 shadow-sm"
            @click="onRemoveItem()">
            <Trash2Icon :size="16" />
          </button>
        </el-tooltip>
    ...
    <div class="flex items-center gap-5">
      <div class="gap-4 hidden md:flex items-center">
        ...
        <el-popover placement="top" trigger="click" width="280"
          ...
          <div class="bg-[#0d0d0d]/95 backdrop-blur-2xl">
            <div class="px-5 py-3 border-b border-white/5 bg-white/5">
              <span class="text-[9px] font-bold text-white/40 uppercase tracking-[0.2em]">{{ t('videoEditor.playback.viewportFormat') }}</span>
            </div>
            <div class="p-4 grid grid-cols-2 gap-3 max-h-[280px] overflow-y-auto custom-scrollbar">
              <div v-for="format in formats" :key="format.name" class="flex flex-col gap-2 group cursor-pointer"
                @click="resizeArtboards(format.dimensions)">
                <el-tooltip :content="format.purpose" placement="top" popper-class="cinematic-tooltip">
                  <div
                    ...
                    <img :src="format.preview"
                      class="object-contain max-h-full max-w-full opacity-60 group-hover:opacity-100 transition-opacity" />
                  </div>
                </el-tooltip>
                <span
                  class="text-[9px] font-bold text-white/30 text-center uppercase tracking-widest group-hover:text-white/60 transition-colors">{{
                    format.name }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="flex items-center bg-white/5 rounded-xl border border-white/5 p-1 gap-1 shadow-sm">
          <button
            class="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition-all active:scale-90"
            @click="workspace.changeZoom(workspace?.zoom - 0.05)">
            <ZoomOut :size="14" />
          </button>

          <el-dropdown max-height="240px" @command="(percentage) => workspace.changeZoom(percentage / 100)"
            trigger="click" popper-class="cinematic-dropdown">
            <button
              class="h-7 px-3 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/90 hover:text-white transition-all text-[11px] font-bold font-mono tracking-tighter min-w-[54px] shadow-sm">
              {{ Math.round(workspace?.zoom * 100) }}%
            </button>
            <template #dropdown>
              <el-dropdown-menu class="cinematic-dropdown-menu">
                <div
                  class="px-4 py-2 text-[9px] font-bold text-white/20 uppercase tracking-widest border-b border-white/5 mb-1">
                  {{ t('videoEditor.playback.canvasScale') }}</div>
                <el-dropdown-item v-for="percentage in [10, 15, 20, 25, 50, 75, 100, 125, 150, 200, 300, 400]"
                  :key="percentage" :command="percentage">
      ...
      <!-- <div class="flex items-center bg-white/5 rounded-xl border border-white/5 p-1 gap-1 shadow-sm">
        <button
          class="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition-all active:scale-90"
          @click="editor.setTimelineZoom(editor.timelineZoom - 0.1)">
          <ZoomOut :size="14" />
        </button>

        <el-dropdown max-height="240px" @command="(val) => editor.setTimelineZoom(val)" trigger="click"
          popper-class="cinematic-dropdown">
          <button
            class="h-7 px-3 flex items-center justify-center rounded-lg hover:bg-white/10 text-white/90 hover:text-white transition-all text-[11px] font-bold font-mono tracking-tighter min-w-[54px] shadow-sm">
            {{ Math.round(editor.timelineZoom * 100) }}%
          </button>
          <template #dropdown>
            <el-dropdown-menu class="cinematic-dropdown-menu">
              <div
                class="px-4 py-2 text-[9px] font-bold text-white/20 uppercase tracking-widest border-b border-white/5 mb-1">
                {{ t('videoEditor.playback.timelineZoom') }}</div>
              <el-dropdown-item v-for="val in [0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 3.0, 4.0]" :key="val" :command="val">
                <span class="text-[10px] font-bold font-mono py-1">{{ val * 100 }}%</span>
              </el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      ...
      <div class="flex gap-0.5 relative group/dur shadow-sm">
        <el-popover placement="top-start" trigger="click" width="280"
          popper-class="cinematic-popover p-0 overflow-hidden border-white/10">
          <template #reference>
            <button
              class="flex items-center gap-2.5 h-9 px-4 rounded-l-xl border border-white/5 bg-white/5 hover:bg-white/10 hover:border-white/10 text-white/50 hover:text-white transition-all duration-300 disabled:opacity-30"
              :disabled="disabled">
              <Timer :size="14" class="text-white/30" />
              <span class="text-[10px] font-bold uppercase tracking-widest">{{ t('videoEditor.playback.duration') }}</span>
            </button>
          </template>
          <div class="bg-[#0d0d0d]/95 backdrop-blur-2xl">
            <div class="px-5 py-3 border-b border-white/5 bg-white/5">
              <span class="text-[9px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.playback.globalTimeline') }}</span>
            </div>
            <div class="p-5 flex flex-col gap-4">
              <div class="flex justify-between items-center mb-1">
                <span class="text-[10px] font-black text-white/20 uppercase tracking-widest">{{ t('videoEditor.playback.timeLimit') }}</span>
                <span class="text-[11px] font-bold font-mono text-white/60 bg-white/5 px-2 py-0.5 rounded-md">{{
                  duration.toFixed(1) }}s</span>
              </div>
              <SliderInput :disabled="disabled" :model-value="duration" :min="MIN_DURATION" :max="MAX_DURATION"
                :step="1" @update:model-value="(value) => duration = value" />
            </div>
          </div>
        </el-popover>
        ...
        <el-dropdown @command="d => timeline.set('duration', d)" trigger="click" popper-class="cinematic-dropdown">
          <button
            class="flex items-center justify-center w-7 h-9 rounded-r-xl border border-white/5 bg-white/5 hover:bg-white/10 text-white/30 hover:text-white transition-all disabled:opacity-30"
            :disabled="disabled">
          ...
          <template #dropdown>
            <el-dropdown-menu class="cinematic-dropdown-menu min-w-[124px]">
              <div
                class="px-4 py-2 text-[9px] font-bold text-white/20 uppercase tracking-widest border-b border-white/5 mb-1">
                {{ t('videoEditor.playback.presets') }}</div>
              <el-dropdown-item v-for="item in presetDurations" :key="item.value"
                :disabled="disabled || duration == item.value" :command="item.value">
      ...
      <el-tooltip :content="t('videoEditor.playback.toggleTimeline')" placement="top" popper-class="cinematic-tooltip">
        <button
          ...
          <GanttChart :size="16" class="transition-colors group-hover/btn:text-white"
            :class="editor.timelineOpen ? 'text-brand-primary' : 'text-white/40'" />
          <span class="hidden sm:inline text-[10px] font-black uppercase tracking-widest">{{ t('videoEditor.playback.timeline') }}</span>
          <ChevronUp :size="14" class="transition-transform duration-500 opacity-30 group-hover/btn:opacity-100"
            :class="editor.timelineOpen ? 'rotate-180' : 'rotate-0'" />
        </button>
      </el-tooltip>
    </div>
  </div>
