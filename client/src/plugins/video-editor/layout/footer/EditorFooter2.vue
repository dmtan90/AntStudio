        ...
        <div
            ...
            <div class="flex items-center gap-2">
                <button
                    ...
                    <Magic :size="16" />
                    <span class="text-sm font-bold uppercase tracking-widest hidden sm:inline-block">{{ t('videoEditor.timeline.transitions') }}</span>
                </button>
                ...
                <button
                    ...
                    <ViewList :size="16" />
                    <span class="text-sm">{{ t('videoEditor.timeline.showTiming') }}</span>
                    ...
                    <div
                        ...
                        <div
                            ...
                            <div v-for="sub in allSubtitles" :key="sub.id"
                                ...
                                <span class="truncate">{{ sub.text || t('videoEditor.timeline.subtitle') }}</span>
                            </div>
                            ...
                            <div v-for="vo in voiceoverElements" :key="vo.id"
                                ...
                                <Music :size="8" class="mr-1" /> {{ vo.name || t('videoEditor.timeline.voiceover') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="track-row h-[28px] flex items-center px-0 pl-0 relative">
                        <div
                            ...
                            <div v-for="audio in musicElements" :key="audio.id"
                                ...
                                <Music :size="8" class="mr-1" /> {{ audio.name || t('videoEditor.timeline.audio') }}
                            </div>
                        </div>
                    </div>
                </div>
...
const startEditing = (sub: any) => {
    ...
    editingText.value = sub.text || t('videoEditor.timeline.subtitle');
    ...
    if (editingText.value !== sub.text) {
        ...
        if (canvas) {
            ...
            if (object) {
                ...
                canvas.renderAll();
                editor.onModified();
                toast.success(t('videoEditor.timeline.subtitleUpdated'));
            }
        }
    }
    cancelEditing();
...
const startRecording = async () => {
    try {
        ...
        mediaRecorder.value.onstop = async () => {
            ...
            // Upload to S3
            let audioUrl = '';
            const loadingToast = toast.loading(t('videoEditor.timeline.uploadingVoiceover'));
            try {
                audioUrl = await uploadFile(audioBlob, 'audio');
                toast.success(t('videoEditor.timeline.voiceoverSaved'), { id: loadingToast });
            } catch (e) {
                console.error(e);
                toast.error(t('videoEditor.timeline.voiceoverFailed'), { id: loadingToast });
                return;
            }
            ...
            if (page) {
                ...
                await page.audio.add(audioUrl, t('videoEditor.timeline.voiceover'), false, id);
...
const togglePlay = () => {
    ...
    if (!timeline.value) {
        toast.error(t('videoEditor.timeline.timelineNotInitialized'));
        return;
    }
    if (isPlaying.value) {
