...
<template>
  <div class="h-full w-full flex flex-col cinematic-panel bg-[#0a0a0a]/95 backdrop-blur-xl">
    ...
    <div class="flex items-center justify-between h-14 border-b border-white/5 px-5 bg-white/5 relative overflow-hidden group shrink-0">
      ...
      <h2 class="font-bold text-xs tracking-[0.2em] uppercase text-white/90 relative z-10">{{ t('videoEditor.clip.title') }}</h2>
        ...
        <div class="p-5 flex flex-col gap-8 pb-10">
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.clip.basicShapes') }}</h4>
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.clip.abstract') }}</h4>
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between">
              <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.clip.frames') }}</h4>
            ...
            <div class="flex gap-3 items-center overflow-x-auto scrollbar-hidden relative min-h-[72px] pb-2 -mx-5 px-5">
              ...
              <template v-else>
                <div class="w-full h-16 rounded-2xl border border-dashed border-white/10 flex items-center justify-center bg-white/2">
                   <span class="text-[9px] font-bold text-white/20 uppercase tracking-widest">{{ t('videoEditor.clip.noElements') }}</span>
                </div>
              </template>
            </div>
          </div>
      ...
      <template v-else>
        <div class="px-5 pt-4 flex flex-col gap-6 pb-20">
           ...
           <div class="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-white/5 border border-white/5 w-fit">
              <button class="text-[10px] font-bold text-white/40 hover:text-white transition-colors uppercase tracking-widest" @click="expanded = false">{{ t('videoEditor.clip.shapes') }}</button>
