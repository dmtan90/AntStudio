...
<template>
    <div class="h-full w-full flex flex-col cinematic-panel bg-[#0a0a0a]/95 backdrop-blur-xl">
        ...
        <div
            ...
            <h2 class="font-bold text-xs tracking-[0.2em] uppercase text-white/90 relative z-10">{{ t('videoEditor.audioSettings.title') }}</h2>
            <div class="flex items-center gap-2 relative z-10">
                <button @click="canvas.onChangeAudioProperties(selected, { visible: !selected.visible })"
                    :title="selected?.visible === false ? t('videoEditor.audioSettings.showLayer') : t('videoEditor.audioSettings.hideLayer')"
            ...
            <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-4 p-4 rounded-2xl bg-white/2 border border-white/5">
                    <div class="flex items-center justify-between">
                        <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.audioSettings.volume') }}</h4>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="flex flex-col gap-3">
                        <div class="flex items-center justify-between">
                            <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.audioSettings.fadeIn') }}</h4>
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="flex items-center justify-between">
                            <h4 class="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.audioSettings.fadeOut') }}</h4>
                ...
                <div class="mt-4 p-4 rounded-2xl bg-brand-primary/5 border border-brand-primary/10">
                    <div class="flex flex-col gap-1">
                        <span
                            class="text-[9px] font-black text-brand-primary uppercase tracking-tighter opacity-60">{{ t('videoEditor.audioSettings.clipName') }}</span>
                ...
                <div class="flex flex-col gap-4 p-4 rounded-2xl bg-gradient-to-br from-brand-primary/10 to-transparent border border-brand-primary/20 relative overflow-hidden group/ai">
                    ...
                    <div class="flex items-center gap-2 relative z-10">
                        ...
                        <h4 class="text-[10px] font-bold text-white/50 uppercase tracking-[0.2em]">{{ t('videoEditor.audioSettings.aiUtilities') }}</h4>
                    </div>
                    ...
                    >
                        ...
                        <div class="relative h-full w-full flex items-center justify-center gap-2 px-4">
                            ...
                            <span class="text-[10px] font-black text-white uppercase tracking-[0.1em]">{{ t('videoEditor.audioSettings.generateVoiceover') }}</span>
                        </div>
                    </button>
                    
                    <p class="text-[9px] text-white/30 leading-relaxed relative z-10 italic">
                        {{ t('videoEditor.audioSettings.aiDescription') }}
                    </p>
                </div>
            </div>
