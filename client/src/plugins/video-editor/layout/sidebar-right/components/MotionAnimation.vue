...
import { Plus, Delete, PlayOne, Time } from '@icon-park/vue-next';
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex items-center justify-between px-1">
            <div class="flex flex-col gap-1">
                <h4 class="text-[10px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.animation.motionTracks') }}</h4>
                <span class="text-[9px] text-white/20">{{ t('videoEditor.animation.addKeyframesPath') }}</span>
            </div>
            <div class="flex items-center gap-2">
                <el-tooltip :content="t('videoEditor.animation.clearAll')" placement="top">
                ...
                <button @click="addKeyframe"
                    ...
                    <Plus :size="14" />
                    <span class="text-[10px] font-bold uppercase tracking-widest">{{ t('videoEditor.animation.addKeyframe') }}</span>
                </button>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-2">
            <div v-if="sortedKeyframes.length === 0"
                ...
                <span class="text-[10px] font-bold text-white/20 uppercase tracking-widest text-center px-10">{{ t('videoEditor.animation.noKeyframesYet') }}</span>
            </div>
            ...
            <div v-for="(kf, index) in sortedKeyframes" :key="index"
                ...
                <div class="flex-1 flex flex-col gap-0.5">
                    <div class="flex items-center justify-between">
                        <span class="text-[11px] font-bold text-white/80">{{ t('videoEditor.animation.at') }} {{ formatMediaDuration(kf.time, false)
                            }}</span>
        ...
        <div v-if="sortedKeyframes.length > 0" class="pt-4 border-t border-white/5">
            <button @click="playPreview"
                ...
                <span
                    class="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60 group-hover/play:text-white transition-colors">{{ t('videoEditor.animation.previewMotion') }}</span>
            </button>
        </div>
    </div>
</template>
