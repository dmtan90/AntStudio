  ...
  <div class="flex flex-col gap-3 group">
      ...
      <div class="flex items-center gap-3" v-if="active">
          ...
          >
            ...
            <div class="bg-[#0d0d0d]/95 backdrop-blur-2xl">
              ...
              <div class="px-5 py-3 border-b border-white/5 bg-white/5">
                <span class="text-[9px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.filters.removeColor') }}</span>
              </div>
              
              <div class="p-5 flex flex-col gap-6" ref="popup">
                <div class="space-y-4">
                  <div class="flex items-center justify-between">
                    <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.fill.color') }}</Label>
                    <el-color-picker v-model="removeColor.color" color-format="hex" :predefine="colors" @change="onChange" />
                  </div>
                  <div class="space-y-3">
                    <div class="flex justify-between">
                        <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.filters.tolerance') }}</Label>
        ...
        <template v-else-if="name == 'BlendColor'">
          ...
          >
            ...
            <div class="bg-[#0d0d0d]/95 backdrop-blur-2xl">
              <div class="px-5 py-3 border-b border-white/5 bg-white/5">
                <span class="text-[9px] font-bold text-white/40 uppercase tracking-widest">{{ t('videoEditor.filters.blendControls') }}</span>
              </div>
              
              <div class="p-5 flex flex-col gap-6">
                <div class="space-y-5">
                  <div class="flex items-center justify-between gap-4">
                    <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.fill.mode') }}</Label>
                  ...
                  <div class="space-y-3">
                    <div class="flex justify-between">
                        <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.filters.opacity') }}</Label>
    ...
    <div v-if="active && (name == 'Noise' || name == 'Pixelate' || name == 'Blur' || name == 'FilmGrain' || name == 'Vignette' || name == 'ChromaticAberration' || !excludeSliders.includes(name))" class="pl-8 pr-2">
        ...
        <div v-else-if="name == 'Vignette'" class="space-y-4">
            <div class="space-y-2">
                <Label class="text-[9px] text-white/30 uppercase tracking-[0.2em]">{{ t('videoEditor.filters.intensity') }}</Label>
    ...
    <template v-if="name == 'Duotone' && active && doutone">
      <div class="flex flex-col gap-4 pl-8 py-3 bg-white/5 rounded-2xl border border-white/5 mt-1 mx-2 p-4">
        <div class="flex items-center justify-between">
          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.filters.light') }}</Label>
          <div class="flex items-center gap-3">
        ...
        <div class="flex items-center justify-between">
          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.filters.dark') }}</Label>
          <div class="flex items-center gap-3">
