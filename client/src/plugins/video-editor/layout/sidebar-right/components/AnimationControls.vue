...
<template>
  <div class="flex flex-col animation-controls gap-5 bg-white/5 p-5 rounded-2xl border border-white/5 relative overflow-hidden group">
    ...
    <div class="flex items-center justify-between gap-4 relative z-10">
      <Label :class="cn('text-[10px] font-bold text-white/40 uppercase tracking-widest', disabled || animation?.disabled?.easing ? 'opacity-30' : 'opacity-100')">{{ t('videoEditor.animation.easingType') }}</Label>
      <el-select v-model="easing" :disabled="disabled || animation?.disabled?.easing" class="cinematic-select w-[130px]" popper-class="cinematic-popover">
        <el-option v-for="e in easings" :key="e.value" :label="t('videoEditor.animation.easings.' + e.value)" :value="e.value" />
      </el-select>
    </div>

    <template v-if="easing !== 'spring'">
      <div class="flex items-center justify-between gap-4 relative z-10">
        <Label :class="cn('text-[10px] font-bold text-white/40 uppercase tracking-widest', disabled || animation?.disabled?.duration ? 'opacity-30' : 'opacity-100')">{{ t('videoEditor.animation.durationS') }}</Label>
    ...
    <template v-else>
      <div class="flex items-center justify-between gap-6 mt-1 relative z-10">
        <Label :class="cn('text-[10px] font-bold text-white/40 uppercase tracking-widest', disabled ? 'opacity-30' : 'opacity-100')">{{ t('videoEditor.animation.springPhysics') }}</Label>
        <el-popover width="300px" trigger="click" placement="bottom-end" popper-class="cinematic-popover p-0 overflow-hidden border-white/10">
          ...
          <section class="flex flex-col bg-[#0d0d0d]/95 backdrop-blur-2xl">
            ...
            <div class="px-5 py-4 border-b border-white/5 bg-white/5">
                <span class="text-[10px] font-bold text-white/60 uppercase tracking-widest">{{ t('videoEditor.animation.adjustPhysics') }}</span>
            </div>
                ...
                <div class="space-y-5">
                   <div>
                      <div class="flex justify-between mb-2">
                          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.animation.mass') }}</Label>
                   ...
                   <div>
                      <div class="flex justify-between mb-2">
                          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.animation.stiffness') }}</Label>
                    ...
                    <div>
                      <div class="flex justify-between mb-2">
                          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.animation.damping') }}</Label>
                   ...
                   <div>
                      <div class="flex justify-between mb-2">
                          <Label class="text-[10px] font-bold text-white/30 uppercase tracking-widest">{{ t('videoEditor.animation.velocity') }}</Label>
    ...
    <div v-if="FabricUtils.isTextboxElement(selected)" class="flex items-center justify-between gap-4 relative z-10">
      <Label :class="cn('text-[10px] font-bold text-white/40 uppercase tracking-widest', disabled || animation?.disabled?.text ? 'opacity-30' : 'opacity-100')">{{ t('videoEditor.animation.textStyle') }}</Label>
      <el-select v-model="text" :disabled="disabled || animation?.disabled?.text || (selected as any)?.type !== 'textbox'" class="cinematic-select w-[130px]" popper-class="cinematic-popover">
        <el-option v-for="type in ['letter', 'word', 'line']" :key="type" :label="t(`videoEditor.animation.textStyles.${type}`)" :value="type" />
      </el-select>
    </div>
  </div>
</template>
