...
const t = (key: string, params?: Record<string, unknown>): string => (i18n.global as any).t(key, params);
...
export const useEditorStore = defineStore('editor', {
  ...
  getters: {
    ...
    exportStatusText(state): string {
      ...
      switch (exporting) {
        case ExportProgress.CaptureVideo: return t('studio.videoEditor.export.extractingFrames');
        case ExportProgress.CaptureAudio: return t('studio.videoEditor.export.processingAudio');
        case ExportProgress.CompileVideo: return t('studio.videoEditor.export.compilingVideo');
        case ExportProgress.Completed: return t('studio.videoEditor.export.completed');
        case ExportProgress.Error: return t('studio.videoEditor.export.error');
        case ExportProgress.UploadVideo: return t('studio.videoEditor.export.uploading');
        default: return t('studio.videoEditor.export.loading');
      }
    },
    blob(state) {
  ...
  actions: {
    ...
    async splitSceneAtPlayhead() {
      ...
      if (seek > 500 && seek < duration - 500) {
        ...
        console.warn(t('studio.videoEditor.messages.splitError'));
      }
    },
    ...
    destroy() {
      if ((this as any)._throttledSave?.cancel) {
        (this as any)._throttledSave.cancel();
      }
    ...
    autoSave() {
      if (!(this as any)._throttledSave) {
        (this as any)._throttledSave = throttle(async () => {
