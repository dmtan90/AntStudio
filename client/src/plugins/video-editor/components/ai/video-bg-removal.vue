...
import { AutoWidth as ChevronsLeftRight, Magic, VideoFile, Close, Plus } from '@icon-park/vue-next';
...
const { selectionActive: selected, canvas } = storeToRefs(useCanvasStore());
      ...
      page.elements.forEach((el: any) => {
        if (el.type === 'video') {
          ...
          if (!mediaMap.has(url)) {
            ...
            mediaMap.set(url, { url, fileName: match?.fileName || (url.split('/').pop()?.split('?')[0] || 'Untitled'), key: match?.key || url });
          }
        }
      });
    }
...
const updateEntry = () => {
    ...
    const key = selectedVideoId.value || sel?.name || '';
...
const handleRemoveBackground = async () => {
  try {
    const source = selectedVideoId.value || (selected.value as any)?.src;
    if (!source) {
    ...
    if(!rmbgAI.getModelInfo().isLoaded){
      toast.info("Loading AI Engine (WebGPU)...");
      await rmbgAI.initializeModel(WEBGPU_MODEL_ID);
    }
    
    const cacheKey = selectedVideoId.value || (selected.value as any)?.name || 'temp_video_bg_removal';
    const blob = await rmbgAI.removeVideoBackground(resolvedOriginal, cacheKey);
...
const handleAdd = () => {
  ...
  if (lastPage) {
      lastPage.onAddVideoFromSource(entry.value.modified, {});
    ...
    <div class="flex flex-col gap-6 p-4">
        <div class="p-4 rounded-2xl bg-brand-primary/5 border border-brand-primary/10 relative overflow-hidden group">
            ...
            <div class="flex items-center gap-2 mb-3">
                <VideoFile :size="14" class="text-brand-primary" />
                <span class="text-[10px] font-bold text-white/50 uppercase tracking-[0.2em]">Video BG Removal</span>
            </div>
            <p class="text-[10px] text-white/40 leading-relaxed italic">
                AI will extract the subject from every frame of your video. Processing happens entirely in your browser.
            </p>
        </div>

        <div class="flex flex-col gap-3 px-1">
            <label class="text-[10px] text-white/40 font-bold uppercase tracking-widest">Source Video</label>
            <el-select v-model="selectedVideoId" class="cinematic-select" placeholder="Choose a video from timeline" clearable>
        ...
        <div class="w-full relative h-[240px] rounded-2xl overflow-hidden bg-black/40 border border-white/5" ref="refContainer" v-loading="loading">
            <template v-if="entry">
                <div class="bg-transparent-pattern h-full">
                    <video :src="entry.modified" class="w-full h-full object-contain" loop autoplay muted />
                </div>
                <div class="bg-transparent-pattern absolute inset-0 overflow-hidden" :style="{ width: position + 'px', height: dimensions.height + 'px' }">
                    <video :src="entry.original" class="w-full h-full object-contain object-left-top" loop autoplay muted />
                </div>
                <VueDraggable
            ...
            <div v-else class="flex flex-col items-center justify-center h-full p-8 text-center bg-white/[0.02]">
                <VideoFile :size="32" class="text-white/10 mb-4" />
                <p class="text-[11px] text-white/30 font-bold uppercase tracking-widest">Ready for extraction</p>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
            ...
            >
                ...
                <span class="text-xs font-black uppercase tracking-widest">Mask Video</span>
            </el-button>
            ...
            <template v-else>
                ...
                <button 
                    ...
                    @click="entry = null; selectedVideoId = ''"
                >
                    Reset Analysis
                </button>
            </template>
