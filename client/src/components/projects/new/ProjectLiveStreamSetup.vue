    ...
    <div class="setup-container glass-card">
      ...
      <div v-if="mode === 'restream'" class="video-selection mt-8 animate-slide-up">
         ...
         <div class="grid grid-cols-1 gap-4 mt-4">
            <el-select v-model="selectedProjectId" placeholder="Select a finished project..." class="w-full glass-input">
      ...
      <div class="platform-selection mt-10">
         ...
         <div v-else-if="!platforms.length" class="empty-notif p-6 text-center border-2 border-dashed border-white/10 rounded-2xl">
            <p class="opacity-50 mb-4">No platforms connected yet.</p>
            <router-link to="/platforms" class="primary-btn secondary inline-block">Connect Platforms First</router-link>
         </div>
         <div v-else class="platforms-list grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div v-for="p in platforms" :key="p._id" 
                 class="p-pill glass-selectable" :class="{ active: selectedPlatforms.includes(p._id) }"
                 @click="togglePlatform(p._id)">
...
.mode-card {
  ...
  h3 { font-size: 20px; font-weight: 800; color: #fff; }
  p { font-size: 14px; opacity: 0.5; line-height: 1.5; }
...
.p-pill {
   ...
   &.active { color: #3b82f6; border-color: rgba($primary-rgb, 0.4); }
}
</style>
