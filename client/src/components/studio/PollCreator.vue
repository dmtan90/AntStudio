...
<template>
    <div class="p-6 space-y-6">
        <div class="flex items-center justify-between">
            <h3 class="text-xs font-black uppercase tracking-widest opacity-40">{{ $t('studio.interactions.createPoll') }}</h3>
        ...
        <div class="space-y-4">
            <div class="space-y-2">
                <label class="text-[10px] font-bold uppercase opacity-60">{{ $t('studio.interactions.pollQuestion') }}</label>
                <el-input 
                    v-model="question" 
                    :placeholder="$t('studio.interactions.pollQuestionPlaceholder')"
                    type="textarea"
                    :rows="2"
                    class="glass-input"
                />
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between">
                    <label class="text-[10px] font-bold uppercase opacity-60">{{ $t('studio.interactions.pollOptions') }}</label>
                    <button 
                    ...
                    >
                        <plus size="12" /> {{ $t('studio.interactions.addOption') }}
                    </button>
                </div>
                ...
                <div class="space-y-2">
                    <div v-for="(opt, idx) in options" :key="idx" class="flex gap-2">
                        <el-input 
                            v-model="opt.label" 
                            :placeholder="$t('studio.interactions.optionLabel', { n: idx + 1 })"
                            size="small"
        ...
        <el-button 
            ...
            @click="handleCreate"
        >
            {{ $t('studio.interactions.launchPoll') }}
        </el-button>
    </div>
</template>
