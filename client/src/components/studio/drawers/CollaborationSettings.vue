    ...
    <div class="collaboration-settings flex flex-col gap-6 animate-in">
        <section>
            ...
            <div class="p-5 rounded-3xl bg-blue-500/5 border border-blue-500/10 mb-6">
                ...
                <div v-if="activeCollaborators.length" class="collaborators-list space-y-3">
                    <div v-for="c in activeCollaborators" :key="c.id"
                        ...
                        <div
                            class="w-8 h-8 rounded-full bg-blue-400/20 border border-blue-400/30 flex items-center justify-center">
                            <span class="text-[10px] font-black">{{ (c.name || '?').charAt(0) }}</span>
                        </div>
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <span class="text-[8px] opacity-30 uppercase font-black">Connected</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div v-else class="text-center py-4 border border-dashed border-white/10 rounded-2xl">
                    <p class="text-[10px] opacity-20 italic">No other co-hosts in room</p>
                </div>
            </div>
        </section>
    </div>
