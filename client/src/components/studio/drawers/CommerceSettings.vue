    ...
    <div class="commerce-settings flex flex-col gap-8 animate-in">
        ...
        <section>
            <div class="flex justify-between items-center mb-4">
                <h4 class="text-xs font-black opacity-30 uppercase tracking-widest">{{ $t('studio.drawers.economy.storefrontSync') }}</h4>
                <button class="text-[10px] text-blue-500 font-bold flex items-center gap-1"
                    ...
                    <fire class="mr-1" /> {{ isFlashDeal ? $t('studio.drawers.economy.endFlash') : $t('studio.drawers.economy.flashDeal') }}
                </button>
            </div>

            <div class="grid grid-cols-1 gap-3">
                <div v-for="p in liveProducts" :key="p._id || p.id"
                    ...
                    :class="{ 'border-blue-500 bg-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.1)]': activeProductId === (p._id || p.id) }"
                    @click="$emit('toggle-product', p._id || p.id)">
                    <img :src="p.image" class="w-14 h-14 rounded-xl object-cover border border-white/10" />
                    <div class="flex-1 min-w-0">
                        <p class="text-[10px] font-black uppercase tracking-tight truncate">{{ p.name }}</p>
                    ...
                    <div class="status-dot" :class="{ active: activeProductId === (p._id || p.id) }"></div>
                </div>
            </div>
        </section>
        ...
        <!-- Sales Performance -->
        <section>
            <h4 class="text-xs font-black opacity-30 uppercase tracking-widest mb-4">{{ $t('studio.drawers.economy.salesIntelligence') }}</h4>
            <div class="stats-grid grid grid-cols-2 gap-3">
                <div
                    ...
                    <p class="text-[8px] opacity-30 uppercase font-black mb-1">{{ $t('studio.drawers.economy.totalOrders') }}</p>
                ...
                <div
                    ...
                    <p class="text-[8px] opacity-30 uppercase font-black mb-1">{{ $t('studio.drawers.economy.revenue') }}</p>
