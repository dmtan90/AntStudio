    ...
    <div class="engagement-settings flex flex-col gap-8 animate-in">
        ...
        <section>
            <div class="flex justify-between items-center mb-4">
                <h4 class="text-xs font-black opacity-30 uppercase tracking-widest">{{ $t('studio.drawers.engagement.livePolls') }}</h4>
                <button v-if="!activePoll" class="text-[10px] text-blue-500 font-bold hover:text-blue-400"
                    @click="$emit('start-poll')">{{ $t('studio.drawers.engagement.startNew') }}</button>
                <button v-else class="text-[10px] text-red-500 font-bold animate-pulse">{{ $t('studio.drawers.engagement.ending') }}</button>
              </div>
            ...
            <div v-else-if="showCreatePoll" class="p-6 bg-white/5 border border-white/10 rounded-3xl animate-in">
                <div class="mb-4">
                    <label class="text-[8px] font-black opacity-30 uppercase block mb-2">{{ $t('studio.drawers.engagement.question') }}</label>
                    <input v-model="newPoll.question"
                        ...
                        :placeholder="$t('studio.drawers.engagement.questionPlaceholder')" />
                </div>
                <div class="space-y-2 mb-4">
                    <label class="text-[8px] font-black opacity-30 uppercase block mb-1">{{ $t('studio.drawers.engagement.options') }}</label>
                    <input v-for="(opt, i) in newPoll.options" :key="i" v-model="newPoll.options[i]"
                        ...
                        :placeholder="$t('studio.drawers.engagement.optionPlaceholder', { n: i + 1 })" />
                </div>
                <div class="flex gap-2">
                    <button class="flex-1 py-2 rounded-xl bg-blue-500 text-white text-[10px] font-black uppercase"
                        @click="handleStartPoll">{{ $t('studio.drawers.engagement.launchPoll') }}</button>
                    <button class="px-4 rounded-xl bg-white/5 text-[10px] font-bold uppercase"
                        @click="showCreatePoll = false">{{ $t('common.cancel') }}</button>
                </div>
            </div>
            <div v-else class="empty-notif p-8 text-center border border-dashed border-white/10 rounded-3xl">
                ...
                <p class="opacity-30 text-[10px] uppercase font-black tracking-widest mb-4">{{ $t('studio.drawers.engagement.noActivePoll') }}
                </p>
                <button
                    ...
                    {{ $t('studio.drawers.engagement.createNewPoll') }}
                </button>
            </div>
        </section>
        ...
        <!-- Q&A Queue -->
        <section>
            <h4 class="text-xs font-black opacity-30 uppercase tracking-widest mb-4">{{ $t('studio.drawers.engagement.qaQueueHeader') }}</h4>
