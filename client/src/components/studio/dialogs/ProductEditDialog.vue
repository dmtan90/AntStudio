  ...
  >
    <div class="p-6 space-y-5">
      <div class="grid grid-cols-1 gap-4">
        ...
        <div class="input-group">
          <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Purchase URL</label>
          <div class="flex gap-2">
            <el-input v-model="form.inventoryUrl" placeholder="https://store.example.com/product..." class="glass-input flex-1" />
            <el-button 
              :loading="extracting" 
              @click="handleExtract"
            ...
            >
              <template v-if="!extracting">
                ...
                <span class="text-[10px] font-black uppercase">Auto Fill</span>
              </template>
              <template v-else>
                <span class="text-[10px] font-black uppercase">Analyzing...</span>
              </template>
            </el-button>
          </div>
          <p class="text-[8px] opacity-25 mt-1.5">Paste product link. Auto Fill extracts details with Gemini AI.</p>
        </div>

        <div class="input-group">
          <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Product Name</label>
          <el-input v-model="form.name" placeholder="Enter product name..." class="glass-input" />
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div class="input-group">
            <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Price</label>
            <el-input-number v-model="form.price" :min="0" class="glass-input w-full" :controls="false" />
          </div>
          <div class="input-group">
            <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Stock</label>
        ...
        <div class="input-group">
          <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Product Images</label>
          <div class="flex gap-2 mb-3">
            <el-input v-model="newImageUrl" placeholder="Paste image URL..." class="glass-input flex-1" size="small" @keyup.enter="addImage" />
          ...
          <div v-if="form.images.length > 0" class="grid grid-cols-5 gap-2">
            ...
            >
              ...
              <span v-if="idx === 0" class="absolute top-0.5 left-0.5 text-[7px] font-black bg-violet-500/90 px-1.5 py-0.5 rounded-br-lg rounded-tl-lg uppercase">Main</span>
            </div>
          </div>
          <p v-else class="text-[8px] opacity-25">No images added. Add URLs above or use Auto Fill.</p>
        </div>

        <div class="input-group">
          <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Description</label>
          <el-input
        ...
        <!-- Features Section -->
        <div class="input-group">
          <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Key Features</label>
        ...
        <div class="space-y-4">
          <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-violet-400">Brand Identity</h3>
          
          <div class="grid grid-cols-2 gap-4">
            <div class="input-group">
              <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Brand Name</label>
              <el-input v-model="form.brand_name" placeholder="Brand name..." class="glass-input" />
            </div>
            <div class="input-group">
              <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Brand Logo URL</label>
              <el-input v-model="form.brand_logo" placeholder="Logo image URL..." class="glass-input" />
            </div>
          </div>

          <div class="input-group">
            <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Brand Slogan</label>
            <el-input v-model="form.brand_slogan" placeholder="Brand tagline..." class="glass-input" />
          </div>

          <div class="input-group">
            <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Ad Video URL (MP4)</label>
            <el-input v-model="form.video" placeholder="Video URL or path..." class="glass-input">
               <template #suffix>
                  <video-file theme="outline" size="14" class="opacity-50" />
               </template>
            </el-input>
            <p v-if="form.video" class="text-[8px] text-blue-400 mt-1 cursor-pointer hover:underline" @click="openPreview">Preview Video</p>
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div class="input-group">
              <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Primary Colors</label>
            ...
            <div class="input-group">
              <label class="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2 block">Secondary Colors</label>
              <div class="flex items-center gap-2">
    ...
    <template #footer>
      <div class="flex justify-end gap-3 p-6 border-t border-white/5 bg-white/[0.02]">
        <el-button @click="isVisible = false" class="glass-btn secondary text-[10px] font-black uppercase tracking-widest">Cancel</el-button>
        <el-button 
          type="primary" 
          @click="handleSave" 
          :loading="loading"
...
const handleExtract = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      toast.success(`Extracted! ${form.value.images.length} image(s) and metadata found.`);
    } else {
      toast.error('Could not extract product information.');
    }
  } catch (e: any) {
