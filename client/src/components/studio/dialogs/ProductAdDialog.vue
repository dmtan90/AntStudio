    ...
    <div class="p-6 min-h-[500px] flex flex-col">
      ...
      <div class="flex justify-between items-center mb-8 px-4">
        <div>
          ...
          <p class="text-[10px] text-gray-400 font-medium">Step {{ step }} of 5</p>
        </div>
        ...
        <Transition name="slide-fade" mode="out-in">
          ...
          <div v-if="step === 1" key="step1" class="space-y-6 h-full flex flex-col justify-center">
            <div class="input-group max-w-lg mx-auto w-full">
              <label class="block text-[10px] font-black uppercase tracking-widest text-blue-400 mb-4 text-center">Analyze Product</label>
              ...
              <div class="text-center">
                 <p class="text-[10px] uppercase opacity-30 mb-4">- OR -</p>
          ...
          <div v-else-if="step === 2" key="step2" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
              <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                   <div class="space-y-4">
                      ...
                      <div>
                        <label class="block text-[8px] font-black opacity-30 uppercase mb-2">Brand Slogan</label>
                        <input v-model="extractedData.brand.slogan" type="text" class="glass-input w-full" placeholder="Optional tagline..." />
                      </div>
                   </div>
                   <div class="flex flex-col">
                      <label class="block text-[8px] font-black opacity-30 uppercase mb-2">Brand Logo</label>
                      <div class="flex gap-3 items-start">
                         <div class="w-20 h-20 rounded-xl bg-black/40 border border-white/5 overflow-hidden flex items-center justify-center p-2 group relative">
                            ...
                            <div v-else class="text-[8px] font-black opacity-20 uppercase">No Logo</div>
                         </div>
                         <div class="flex-1">
                            <input v-model="extractedData.brand.logo" type="text" class="glass-input w-full text-[10px]" placeholder="Logo URL..." />
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-6">
                   <div>
                      <label class="block text-[8px] font-black opacity-30 uppercase mb-3 text-blue-400">Key Features</label>
                      ...
                      <div class="flex gap-2">
                         <input v-model="newFeature" type="text" placeholder="Add feature..." class="glass-input flex-1 text-[10px] py-2" @keyup.enter="addFeature" />
                   ...
                   <div>
                      <label class="block text-[8px] font-black opacity-30 uppercase mb-3 text-blue-400">Color Palette</label>
                      <div class="space-y-4">
                         <div>
                            <p class="text-[6px] font-black uppercase tracking-widest opacity-30 mb-2">Primary</p>
                            <div class="flex gap-2">
                         ...
                         <div>
                            <p class="text-[6px] font-black uppercase tracking-widest opacity-30 mb-2">Secondary</p>
                            <div class="flex gap-2">
              ...
              <div class="bg-black/20 rounded-2xl p-4 border border-white/5">
                 <label class="block text-[8px] font-black opacity-30 uppercase mb-3">Select Product Image</label>
          ...
          <div v-else-if="step === 3" key="step3" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
               <div class="md:col-span-1 space-y-4">
                  <label class="block text-[10px] font-black uppercase tracking-widest text-blue-400 mb-2">Target Format</label>
                  <div class="space-y-3">
                    <div 
                      v-for="f in formats" 
                      :key="f.id" 
                ...
                <div class="md:col-span-2 flex flex-col h-full">
                  <div class="flex items-center justify-between mb-4">
                    <label class="block text-[10px] font-black uppercase tracking-widest text-blue-400">Select Template</label>
                  ...
                  <div class="template-grid grid grid-cols-3 gap-3 flex-1 overflow-y-auto pr-1 custom-scrollbar min-h-[300px]">
                     ...
                     <template v-else>
                        ...
                        <div v-if="filteredTemplates.length === 0" class="col-span-3 flex flex-col items-center justify-center py-12 text-center opacity-40">
                           ...
                           <p class="text-[10px] uppercase font-black tracking-widest">No templates found for this ratio</p>
                        </div>
                     </template>
                  </div>
                  ...
                  <div class="mt-4 flex justify-between items-center bg-white/[0.02] border border-white/5 p-3 rounded-2xl">
                    <span class="text-[8px] font-black uppercase text-gray-500">Total: {{ totalTemplates }}</span>
                    <el-pagination
          ...
          <div v-else-if="step === 4" key="step4" class="h-full flex flex-col justify-center items-center space-y-8">
             <div class="text-center">
                ...
                <h2 class="text-xl font-black text-white uppercase mb-2 tracking-tight">Ready to Launch</h2>
                <p class="text-gray-400 text-xs max-w-sm mx-auto">Your high-converting ad configuration is ready. Choose an enhanced workflow.</p>
             </div>
             ...
             <div class="grid grid-cols-2 gap-6 w-full max-w-2xl px-4">
                <button class="p-6 rounded-3xl bg-white/5 border border-white/5 hover:border-blue-500/30 hover:bg-white/[0.08] transition-all group text-left" @click="launchEditor(false)">
                   ...
                   <h3 class="text-sm font-black text-white uppercase mb-1">Editor Studio</h3>
                   <p class="text-[10px] text-gray-400 leading-relaxed">Full control. Edit every frame, transition, and brand asset manually.</p>
                </button>
                ...
                <button class="p-6 rounded-3xl bg-gradient-to-br from-blue-600 to-blue-500 border border-blue-400 shadow-[0_10px_30px_rgba(59,130,246,0.3)] hover:scale-[1.02] transition-all text-left group" @click="launchEditor(true)">
                   ...
                   <h3 class="text-sm font-black text-white uppercase mb-1">Quick Render</h3>
                   <p class="text-[10px] text-blue-100 leading-relaxed">AI builds and exports the video in the background instantly.</p>
                </button>
             </div>
          </div>
          <!-- Step 5: Render -->
          <div v-else-if="step === 5" key="step5" class="h-full flex flex-col justify-center items-center space-y-8">
            ...
            <template v-if="isSuccess">
              ...
              <div class="text-center w-full max-w-xl px-12 animate-in fade-in zoom-in duration-500">
                ...
                <h2 class="text-3xl font-black text-white uppercase tracking-tight mb-2">Ad Ready!</h2>
                <p class="text-xs text-gray-400 uppercase tracking-widest font-black mb-6">High-Conversion Creative Generated</p>
                ...
                <div class="relative rounded-3xl overflow-hidden bg-black border border-white/5 shadow-2xl mb-8 group">
                  ...
                  <div v-else class="w-full h-full flex flex-col items-center justify-center space-y-3">
                    ...
                    <p class="text-[8px] font-black uppercase tracking-widest opacity-40">Preparing Preview...</p>
                    <p class="text-[10px] font-bold text-white group-hover:text-blue-400 transition-colors">view_ad_creative.mp4</p>
                  </div>
                  <div class="flex gap-2 justify-center">
                    <button class="p-5 rounded-2xl bg-white/5 border border-white/5 hover:border-purple-500/30 transition-all cursor-pointer group" @click="openLink(`/p/${props.product?._id || props.product?.id}`)">
                      <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-[10px] font-black uppercase text-gray-400">Landing Page</span>
                      </div>
                    </button>
                    <button @click="publishToProduct" :disabled="isPublishing" class="p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-green-500/30 transition-all group">
                      <div class="w-8 h-8 rounded-lg bg-green-500/10 flex items-center justify-center text-green-400 mx-auto mb-2 group-hover:bg-green-500 group-hover:text-white transition-all">
                        ...
                        <span class="text-[9px] font-black uppercase text-gray-400 group-hover:text-white">Publish</span>
                      </div>
                    </button>
                ...
                <div class="p-6 rounded-3xl bg-blue-500/10 border border-blue-500/20 text-left flex items-start gap-4 mb-8">
                  ...
                  <div>
                    <h4 class="text-xs font-black text-white uppercase mb-1">LiveStudio Integrated</h4>
                    <p class="text-[10px] text-gray-400 leading-relaxed">This ad has been automatically added to your product's ad creative slot. It will appear for viewers when this product is featured in your live streams.</p>
                  </div>
                </div>
                <div class="flex justify-center gap-4">
                  <button @click="isVisible = false" class="px-10 py-4 rounded-2xl bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-widest border border-white/10 transition-all">Done</button>
                </div>
              </div>
            </template>
              ...
              <div class="text-center w-full max-w-lg px-8">
                <div class="flex items-center justify-between mb-4">
                  <div class="flex items-center gap-3 text-left">
                    ...
                    <div>
                      <h2 class="text-xl font-black text-white uppercase tracking-tight">Rendering Ad</h2>
...
const stepTitles = ['Analyze Product', 'Review Details', 'Select Template', 'Finalize'];
...
const analyzeProduct = async () => {
  ...
  try {
    ...
    toast.error('AI could not analyze this URL. Please fill manually.');
...
const launchEditor = async (headless: boolean) => {
  ...
  template.value.name = finalProduct.name + " Ad";
      ...
      if(productId){
        try{
          ...
          if (data?.publish?.s3Key && productId) {
            ...
            toast.success('Ad video published and linked to product successfully!');
          }
        }catch(error){
          console.error('Error publishing ad video:', error);
          toast.error('Failed to publish ad video');
        }
      }
    }
    toast.success('Ad rendered and downloaded successfully!');
  } catch (err) {
     console.error('Headless render failed:', err);
     toast.error('Failed to render ad. Please try the Editor Studio.');
  } finally {
...
const publishToProduct = async () => {
    ...
    try {
        const productId = props.product?._id || props.product?.id;
        if (productId && adVideoKey.value) {
            await api.put(`/commerce/products/${productId}`, { 
                video: adVideoKey.value 
            });
            toast.success('Product updated with new ad video!');
        } else {
            toast.error('Product or video not found');
        }
    } catch (err) {
...
const shareVideo = () => {
    if (navigator.share && videoBlob.value) {
        ...
        toast.info('Landing page link copied to clipboard!');
  }
};
</script>
