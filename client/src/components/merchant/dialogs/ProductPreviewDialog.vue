                ...
                <div class="lg:w-[45%] p-6 md:p-8 lg:p-10 bg-[#0a0a0c] overflow-y-auto lg:max-h-[80vh] space-y-6 border-l border-white/5">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col min-w-0">
                            <span class="text-[9px] font-black uppercase tracking-[0.15em] text-blue-400 truncate">{{ product.brand_slogan || t('merchant.previewDialog.defaultSlogan') }}</span>
                            <span class="text-sm font-black uppercase tracking-tight truncate">{{ product.brand_name || t('merchant.previewDialog.defaultBrand') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-5 rounded-2xl bg-white/5 border border-white/5 relative overflow-hidden group">
                        ...
                        <div class="relative z-10 flex justify-between items-end">
                            <div>
                                <div class="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1">{{ t('merchant.editDialog.price') }}</div>
                                <div class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">
                                    {{ product.currency }} {{ product.price }}
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1">{{ t('merchant.editDialog.stock') }}</div>
                                <div class="text-lg font-black" :class="product.stock < 5 ? 'text-red-500 animate-pulse' : 'text-white'">
                                    {{ product.stock }} {{ t('merchant.previewDialog.units') }}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="text-center p-3 rounded-xl bg-white/5 border border-white/5">
                            ...
                            <div class="text-[8px] font-black uppercase tracking-widest text-gray-500 mt-0.5">{{ t('merchant.previewDialog.views') }}</div>
                        </div>
                        <div class="text-center p-3 rounded-xl bg-white/5 border border-white/5">
                            <div class="text-lg font-black text-purple-400">{{ product.clicks || 0 }}</div>
                            <div class="text-[8px] font-black uppercase tracking-widest text-gray-500 mt-0.5">{{ t('merchant.previewDialog.clicks') }}</div>
                        </div>
                        <div class="text-center p-3 rounded-xl bg-white/5 border border-white/5">
                            <div class="text-lg font-black text-green-400">{{ product.sales || 0 }}</div>
                            <div class="text-[8px] font-black uppercase tracking-widest text-gray-500 mt-0.5">{{ t('merchant.previewDialog.sales') }}</div>
                        </div>
                    </div>
                    ...
                    <div v-if="product.primary_colors?.length" class="flex items-center gap-2"> 
                        <span class="text-[9px] font-black uppercase tracking-widest text-gray-600 mr-1">{{ t('merchant.previewDialog.brand') }}</span>
                    ...
                    <div class="space-y-3">
                        <button @click="createAd" class="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 border border-blue-500 transition-all font-black text-xs uppercase tracking-widest text-white shadow-lg shadow-blue-600/20 hover:shadow-blue-600/40 hover:-translate-y-0.5">
                            <magic size="16" />
                            {{ t('merchant.previewDialog.launchWizard') }}
                        </button>
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <button @click="handleRemove" class="flex items-center justify-center gap-1.5 py-3 rounded-xl bg-white/5 border border-white/5 hover:bg-red-500/10 hover:border-red-500/30 transition-all group">
                                ...
                                <span class="text-[8px] font-black uppercase tracking-widest text-gray-400 group-hover:text-red-500">{{ t('common.actions.delete') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
