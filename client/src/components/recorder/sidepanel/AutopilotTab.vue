    ...
    <div class="flex flex-col gap-6">
        <div class="panel-section">
            <div class="flex items-center justify-between mb-4">
                <span class="text-[10px] font-bold text-orange-400 uppercase">AI Broadcast Engine</span>
            ...
            <div v-else class="space-y-4">
                ...
                <div class="scene-list flex flex-col gap-2 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                    <div v-for="(scene, idx) in autopilotData.scenes" :key="idx"
                        ...
                        :class="currentSlideIndex === idx ? 'bg-orange-500/10 border-orange-500/30' : 'bg-white/5 border-white/5 hover:border-white/10'"
                        @click="$emit('update:currentSlideIndex', Number(idx))">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[9px] font-black text-orange-400 uppercase">Slide {{ Number(idx) + 1
                                }}</span>
                ...
                <div class="panel-section border-t border-white/5 pt-6">
                    <span class="text-[10px] font-bold text-orange-400 uppercase mb-4 block">AI Presenter</span>
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        <div v-for="av in avatarPresets" :key="av.id"
                            ...
                            :class="selectedAvatar === av.id ? 'bg-orange-500/10 border-orange-500/40 shadow-lg shadow-orange-500/5' : 'bg-white/5 border-white/5 hover:border-white/10'"
                    ...
                    <span class="text-[10px] font-bold text-orange-400 uppercase mb-4 block">Voice Persona</span>
                ...
                <button v-if="!isRecording"
                    class="w-full py-4 rounded-xl bg-orange-500 text-black text-[10px] font-black uppercase tracking-widest hover:bg-orange-400 shadow-xl shadow-orange-500/20 transition-all transform hover:-translate-y-0.5 active:translate-y-0"
