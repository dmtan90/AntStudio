<template>
    <el-dialog 
        ...
        :title="$t('influencers.voiceLibrary.title')" 
        width="850px" 
        append-to-body 
            ...
            <div class="flex flex-col gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                <div class="flex items-center gap-4">
                    <div class="flex-1 flex items-center gap-3 bg-black/20 px-3 py-1.5 rounded-xl border border-white/5">
                        ...
                        <el-input v-model="searchQuery" :placeholder="$t('influencers.voiceLibrary.filterPlaceholder')" class="voice-search-input-inner" />
                    </div>
                    
                    <div class="flex items-center gap-2 px-1">
                        <span class="text-[8px] font-black opacity-30 uppercase tracking-widest mr-2">{{ $t('influencers.voiceLibrary.provider') }}</span>
                        <el-radio-group :model-value="provider" :disabled="true" @update:model-value="$emit('update:provider', $event)" size="small" class="soul-radio-group">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                         <span class="text-[8px] font-black opacity-30 uppercase tracking-widest">{{ $t('influencers.voiceLibrary.language') }}</span>
...
const fetchVoices = async () => {
    loading.value = true;
    try {
        const data = await influencerStore.fetchVoices(props.provider || 'gemini');
...
const handlePreview = async (voice: any) => {
    ...
    if (!srcUrl) {
        ...
        try {
            const previewText = t('influencers.voiceLibrary.tts.preview');
