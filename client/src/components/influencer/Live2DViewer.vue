...
// 1. Dynamic SDK Loading to prevent global performance impact
const loadLive2DSDK = async () => {
    // Both window.Live2D (Cubism 2) and window.Live2DCubismCore (Cubism 3/4) are required
    if ((window as any).Live2DCubismCore && (window as any).Live2D) return;

    return new Promise<void>((resolve, reject) => {
        console.log('[Live2DViewer] Injecting Live2D SDKs...');
        
        // Use absolute paths to ensure loading works on sub-routes
        const scripts = [
            '/models/live2dcubismcore.min.js',
            '/models/live2d.min.js'
        ];
        ...
        scripts.forEach(src => {
            ...
            script.onload = () => {
                ...
                if (loadedCount === scripts.length) {
                    console.log('[Live2DViewer] SDKs loaded successfully');
                    // PIXI requires window.PIXI for the Live2D plugin
...
// 2. Configure ZipLoader to use JSZip
...
const initValue = async () => {
    ...
    try {
        // Ensure SDK is loaded before initializing PIXI/Live2D
        await loadLive2DSDK();
    } catch (e) {
        console.error('[Live2DViewer] SDK Loading failed:', e);
        loading.value = false;
        return;
    }
...
const loadModel = async () => {
    ...
    try {
        ...
        // Result: 75% vertical occupation as requested by user
        const padding = 0.75;
