    ...
    <div ref="container" class="w-full h-full rounded-xl overflow-hidden relative group bg-[#050505]">
        ...
        <div v-if="loading" class="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-md z-10">
            <div class="flex items-center gap-2 px-3 py-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/10">
                ...
                <span class="text-[10px] text-white/70 font-medium tracking-wider uppercase">{{ $t('influencers.neuralLinkEstablishing') }}</span>
            </div>
        </div>
...
const initValue = async () => {
    ...
    // 2. Camera setup (Dynamic Aspect Ratio)
    const isPortrait = props.isPortrait;
    const targetAspect = isPortrait ? (9 / 16) : (16 / 9);
    
    // Internal resolution: 1280x720 (Landscape) or 608x1080 (Portrait, matching 9:16 high-def)
    const baseHeight = isPortrait ? 1080 : 720;
    const baseWidth = Math.round(baseHeight * targetAspect); 
    
    camera = new THREE.PerspectiveCamera(35, targetAspect, 0.1, 100);
    camera.position.set(0, 1.4, isPortrait ? 5.0 : 3.5); 
    ...
    if (canvas.value) {
        ...
        canvas.value.style.objectFit = 'contain'; // Contain ensures no cropping of AI guests in slots
    }
...
const resetView = () => {
    ...
    // Standard 75% vertical occupation for influencers
    const margin = props.isPortrait ? 1.35 : 1.1; 
    let dist = (size.y * margin) / (2 * Math.tan(fov / 2));
    
    dist = Math.max(0.5, Math.min(dist, 20.0));

    console.log(`[VRMViewer] Auto-fitting ${props.isPortrait ? 'Portrait' : 'Landscape'} Feed:`, { center, size, dist });
...
const loadModel = async () => {
    ...
    try {
        ...
        // Aggressive Isolation: Hide rooms/stages embedded in the model
        vrm.scene.traverse((child: any) => {
            if (child.isMesh) {
                const name = (child.name || "").toLowerCase();
                const isBg = name.includes('bg') || name.includes('background') || name.includes('room') || 
                             name.includes('stage') || name.includes('floor') || name.includes('env') ||
                             name.includes('sky') || name.includes('wall') || name.includes('glass') ||
                             name.includes('ground') || name.includes('box') || name.includes('platform');
