<template>
  <el-dialog 
    v-model="visible" 
    :title="t('influencers.music.title')"
    width="900px"
    class="music-dialog"
  >
    <div class="music-content">
      ...
      <div class="flex flex-col md:flex-row gap-4 mb-6 mt-2">
        <div class="flex-1">
          <el-input 
            v-model="searchQuery"
            :placeholder="t('influencers.music.searchPlaceholder')"
            size="large"
            clearable
        ...
        <div v-if="!searchQuery" class="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
          <span class="text-[10px] font-black uppercase tracking-widest text-white/40">{{ t('influencers.music.trending') }}</span>
          <el-select v-model="trendingRegion" size="small" @change="fetchTrending" class="w-32">
            <el-option v-for="(name, code) in t('influencers.music.regions')" :key="code" :label="name" :value="String(code).toUpperCase()" />
          </el-select>
      ...
      <div v-if="searchQuery" class="flex flex-wrap gap-4 mb-6 px-1">
        <el-checkbox v-model="preferCovers" class="!mr-0">{{ t('influencers.music.preferCovers') }}</el-checkbox>
        
        <div class="flex items-center gap-2 ml-auto">
          <span class="text-[10px] font-bold uppercase tracking-wider text-white/30">{{ t('influencers.music.target') }}</span>
          <el-select v-model="language" :placeholder="t('influencers.music.searchLanguage')" class="w-32" size="small">
            <el-option v-for="(name, val) in t('influencers.music.languages')" :key="val" :label="name" :value="val" />
          </el-select>
        </div>

        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold uppercase tracking-wider text-white/30">{{ t('influencers.music.lyricsLabel') }}</span>
          <el-select v-model="lyricsLanguage" :placeholder="t('influencers.music.lyricsLanguage')" class="w-32" size="small">
            <el-option v-for="(name, val) in t('influencers.music.lyricsLanguages')" :key="val" :label="name" :value="val" />
          </el-select>
        </div>
      </div>
      ...
      <div class="results-container min-h-[400px]" v-loading="searching || trendingLoading">
        ...
        <div v-else-if="!searching && !trendingLoading" class="py-20 flex flex-col items-center justify-center text-white/20 border-2 border-dashed border-white/5 rounded-3xl">
          ...
          <span class="text-[10px] font-mono uppercase tracking-[0.3em]">{{ t('influencers.music.noMusicFound') }}</span>
          <p class="text-[9px] mt-2 opacity-30">{{ t('influencers.music.tryDifferentSearch') }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedVideo" class="mt-6 p-4 bg-white/5 rounded-2xl border border-white/10">
      <h3 class="text-sm font-bold mb-3">{{ t('influencers.music.selectedSong') }}</h3>
      <div class="flex items-center gap-4">
        ...
        <el-button 
          ...
          type="primary"
        >
          {{ fetching ? t('influencers.music.fetching') : t('influencers.music.fetchAndApply') }}
        </el-button>
      </div>
    </div>
    ...
    <div v-if="lyricsLines.length > 0" class="mt-6">
      <h3 class="text-sm font-bold mb-3">{{ t('influencers.music.lyricsPreview') }}</h3>
      <div class="lyrics-preview">
        ...
        <p v-if="lyricsLines.length > 10" class="text-xs text-white/40 mt-2">
          {{ t('influencers.music.moreLines', { count: lyricsLines.length - 10 }) }}
        </p>
      </div>
      ...
      <div class="mt-4 grid grid-cols-2 gap-4">
        <div>
          <label class="text-xs text-white/40 mb-2 block">{{ t('influencers.music.animationStyle') }}</label>
          <el-select v-model="lyricsStyle" class="w-full">
            <el-option v-for="(name, val) in t('influencers.music.styles')" :key="val" :label="name" :value="val" />
          </el-select>
        </div>
        <div>
          <label class="text-xs text-white/40 mb-2 block">{{ t('influencers.music.position') }}</label>
          <el-select v-model="lyricsPosition" class="w-full">
            <el-option v-for="(name, val) in t('influencers.music.positions')" :key="val" :label="name" :value="val" />
          </el-select>
        </div>
      </div>
    </div>
...
const searchMusic = async () => {
  if (!searchQuery.value) {
    toast.error(t('influencers.music.toasts.enterQuery'));
    return;
  }
  ...
  try {
    ...
    if (data.length === 0) {
      toast.info(t('influencers.music.toasts.noResults'));
    }
  } catch (error: any) {
...
const fetchMetadataAndLyrics = async () => {
  ...
  try {
    ...
    if (lyricsLines.value.length > 0) {
      toast.success(t('influencers.music.toasts.lyricsFetched'));
      // Auto-confirm selection
      confirmSelection();
    } else {
      toast.warning(t('influencers.music.toasts.noLyrics'));
      confirmSelection();
    }
  } catch (error: any) {
    console.error('Fetch failed:', error);
    toast.error(t('influencers.music.toasts.fetchFailed'));
  } finally {
...
const confirmSelection = () => {
  ...
  visible.value = false;
  toast.success(t('influencers.music.toasts.applied'));
};
