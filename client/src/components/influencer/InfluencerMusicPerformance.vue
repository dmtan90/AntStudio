<template>
  <div class="influencer-music-performance relative w-full h-full bg-black rounded-xl overflow-hidden">
    <!-- Influencer Viewer -->
    <div class="absolute inset-0">
      <InfluencerViewer
        v-if="influencer"
        :model-type="influencer.visual?.modelType || 'vrm'"
        :model-url="influencer.visual?.modelUrl"
        :speaking-vol="audioLevel"
        :aura-enabled="influencer.performanceConfig?.auraEnabled"
        :aura-color="influencer.performanceConfig?.auraColor"
        :particle-type="influencer.performanceConfig?.particleType"
        :particle-density="influencer.performanceConfig?.particleDensity"
        :lighting-preset="influencer.performanceConfig?.lightingPreset || 'vocal_orange'"
        :active-camera-path="influencer.performanceConfig?.activeCameraPath"
        :auto-director-enabled="true"
        :background-url="influencer.visual?.backgroundUrl"
        :emotion="influencer.currentEmotion || influencer.emotion"
        :gesture="influencer.currentGesture || influencer.gesture"
        :animation-config="influencer.animationConfig"
      />
    </div>
    ...
    <div v-if="loading" class="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-xl z-50">
      <div class="flex flex-col items-center gap-4">
        ...
        <span class="text-[10px] font-black uppercase tracking-widest text-blue-400">{{ $t('influencers.musicPerformance.loading') }}</span>
      </div>
    </div>
  </div>
</template>
