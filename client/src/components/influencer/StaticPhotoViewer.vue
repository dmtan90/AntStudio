    ...
    <div ref="container" class="w-full h-full relative group bg-[#050505] rounded-xl overflow-hidden" 
        ...
        <div v-if="loading" class="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-md z-50">
            <div class="flex flex-col items-center gap-4">
                ...
                <span class="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">{{ $t('influencers.neuralLinkEstablishing') }}</span>
            </div>
        </div>
...
let mouthPatchTexture: PIXI.Texture | null = null;
const visemeTextures: Record<string, PIXI.Texture> = {}; // Multi-viseme cache (v4)
let bgSprite: PIXI.Sprite | null = null;
let mouthPatchSprite: PIXI.Sprite | null = null;
...
const VOLUME_SMOOTHING = 0.92;

let atmosphere: AtmosphereManager | null = null;
let faceScale = 1.0;
const clampedVolRef = ref(0);
const animatedVolumeRef = ref(0);
let eyeHeight = 10;
let mouthHeight = 20;
...
let mouthStabilizationStart = 0;
let mouthStabilizationEnd = 0;
...
// Persistent User Transformation
let userX = 0;
let userY = 0;
let userZoom = 1.0;
let userRotation = 0;
...
const setupMesh = (landmarks: any[], tex: PIXI.Texture, scale: number) => {
    ...
    mouthStabilizationStart = points.length / 2;
    ...
    mouthStabilizationEnd = points.length / 2;
    ...
    // 3. Create Custom Hole-Punching Shader (Neural Masking)
    ...
    if (mouthPatchTexture) {
        mouthPatchSprite = new PIXI.Sprite(mouthPatchTexture as any);
        mouthPatchSprite.anchor.set(0.5);
        mouthPatchSprite.zIndex = 3; // Above mesh
        app.stage.addChild(mouthPatchSprite as any);
    }
    ...
    // Add to stage
    mesh.zIndex = 2; // Behind mouth patch (3)
    app.stage.addChild(mesh);
...
const update = (delta: number) => {
    ...
    clampedVolRef.value = Math.min(smoothedVolume, 0.55); // Slightly increased max for O-shape
    ...
    if (app && mesh) {
        ...
        let autoGX = 0;
        let autoGY = 0;
        if (props.gesture === 'look_around' && clampedVolRef.value < 0.05) {
    ...
    // Lip Sync Pre-calculations (Shared scope for mesh & mouth patch)
    clampedVolRef.value = Math.min(smoothedVolume, 0.5);
    animatedVolumeRef.value = Math.sqrt(clampedVolRef.value); 
    const baseMovement = animatedVolumeRef.value * mouthHeight * 0.9;
    ...
    for (let i = 0; i < positions.length; i += 2) {
        ...
        // Dynamic Face Center (better than magic 0.4)
        const mx_center_base = lastLandmarks[13];
        const faceCenterX = textureWidth * displayScale * 0.5;
        const faceCenterY = textureHeight * displayScale * (mx_center_base ? mx_center_base.y : 0.5);
        ...
        // Only warp mouth if volume is high (Legacy fallback removed per user request)
        if (false && smoothedVolume > 0.005) {
            const mouthLM = lastLandmarks[13];
            const mx_center = mouthLM.x * textureWidth * displayScale;
            
            let vWeight = 0; 
            if (MOUTH_CORE.includes(idx)) vWeight = 1.0;
            else if (MOUTH_SOFT.includes(idx)) vWeight = 0.6;
            else if (MOUTH_SHORE.includes(idx)) vWeight = 0.25;
            ...
            if (vWeight > 0) {
                ...
                const puckerFactor = Math.abs(ox - mx_center) / (textureWidth * displayScale * 0.1); 
                const squeezeX = baseMovement * 0.5 * Math.exp(-puckerFactor * puckerFactor);
                if (ox < mx_center) tx += squeezeX; else tx -= squeezeX;
                
                if (idx === L_CORNER || idx === R_CORNER) {
                    tx += (idx === L_CORNER ? 15 : -15) * animatedVolumeRef.value;
                }
            }
        }
        
        // 2. Apply Blink (Always active)
        if (blinkVal > 0.01) {
        ...
        // 3. Expressions (Mouth corners skip warping to favor patch)
        if (false && (idx === L_CORNER || idx === R_CORNER)) {
        ...
        const animatedVol = animatedVolumeRef.value;
        const shape = analyzeVisemeShape(animatedVol);
        const mouthWidth = Math.abs(positions[R_CORNER * 2] - positions[L_CORNER * 2]);
        const deform = computeLipDeform(shape, mouthHeight, mouthWidth);
        
        // Find current vertical center of the mouth to decide UP/DOWN push
        const mouthCenterY = (positions[13 * 2 + 1] + positions[14 * 2 + 1]) / 2;
        const mouthCenterX = (positions[L_CORNER * 2] + positions[R_CORNER * 2]) / 2;

        for (let i = 0; i < positions.length - 1; i += 2) {
            const idx = i / 2;
            let weight = 0;
            if (MOUTH_CORE.includes(idx)) weight = 1.0;
            else if (MOUTH_SOFT.includes(idx)) weight = 1.0; // Keep outer lips sharp too
            else if (MOUTH_SHORE.includes(idx)) weight = 0.3; // Padding for surrounding skin
            else if (idx >= mouthStabilizationStart && idx < mouthStabilizationEnd) weight = 0.6; // stabilization points padding

            if (weight > 0) {
                // 1. Vertical Movement (Open/Close)
                ...
                // 2. Horizontal Movement (Width/Round/Pucker)
                // Move corners outward/inward, and scale other mouth points proportionally
                const relX = (positions[i] - mouthCenterX) / (mouthWidth / 2); // -1 to 1
        ...
        if (mouthPatchSprite) {
            mouthPatchSprite.alpha = 0;
        }

    
    // 6. Update Mouth Interior Overlay Position (Deprecated)
...
watch(() => props.modelUrl, (newUrl) => {
    if (newUrl) {
        console.log('[StaticPhotoViewer] modelUrl changed, re-initializing full app...');
        init();
    }
});

watch(() => props.influencer?.visual?.visemeSpritesheet, (newSpritesheet) => {
    if (newSpritesheet) {
        console.log('[StaticPhotoViewer] visemeSpritesheet changed, reloading textures only...');
        loadVisemes();
    }
});
...
const loadVisemes = async () => {
    ...
    console.log(`[StaticPhotoViewer] Loading Visemes based on: ${spritesheetPath}`);
    ...
    try {
        ...
        const fetchViseme = async (v: string) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    visemeTextures[v] = await PIXI.Texture.fromURL(blobUrl);
                    console.log(`[StaticPhotoViewer] Loaded Viseme: ${v}.svg`);
                }
            } catch (err) {
                console.error(`[StaticPhotoViewer] Failed to load ${v}.svg from ${url}`, err);
            }
        };
        ...
        if (visemeTextures['S']) {
            mouthPatchTexture = visemeTextures['S'];
            if (mouthPatchSprite) mouthPatchSprite.texture = mouthPatchTexture;
        }
    } catch (err) {
        console.error(`[StaticPhotoViewer] Viseme loading error:`, err);
    }
};
...
const init = async () => {
    ...
    try {
        ...
        if (canvas.value) {
            ...
            canvas.value.style.objectFit = 'contain'; 
        }
        ...
        if (app) {
            ...
            // Cap to 15fps — the Delaunay vertex deformation loop iterates 500+
            // vertices/frame; at 60fps this saturates a CPU core unnecessarily.
        ...
        if (detection?.faceLandmarks?.length > 0) {
            ...
            const width = app!.screen.width;
            const height = app!.screen.height;
            // Fit height strategy (100% vertical occupation)
            const finalScale = height / texture.height;
            ...
            if (app) {
                ...
                auraSprite.zIndex = 1; // Behind mesh (2)
