        ...
        <div v-for="(img, idx) in localImages" :key="idx" class="flex flex-col gap-2.5 bg-white/5 p-2.5 rounded-lg border border-white/5 transition-all hover:bg-white/10 hover:-translate-y-0.5">
          <div class="w-full aspect-square rounded-lg overflow-hidden relative bg-black/20 border border-white/10">
             <el-image 
                ...
                :src="getFileUrl(img.s3Key)" 
                :alt="img.name" 
                class="w-full h-full object-cover" 
                fit="contain"
                :preview-src-list="[getFileUrl(img.s3Key)]"
             />
...
const getFileUrl = (path: string | undefined | null) => {
  if (!path) return ''
  // Enforce S3 proxy usage for all assets
  return `/api/s3/${path}`
}
...
const processQueue = async () => {
    ...
    try {
        ...
        const { data } = await $fetch(`/api/projects/${props.projectId}/assets/generate`, {
            method: 'POST',
            body: { 
