...
import { PlatformAuthService } from '../system/PlatformAuthService.js';
import { aiPerformanceService } from '../ai/AIPerformanceService.js';
import { geminiLiveService } from '../ai/GeminiLiveService.js';
import { audienceInteractionService } from '../ai/AudienceInteractionService.js';
import { gamificationService } from './GamificationService.js';
import { virtualEconomyService } from './VirtualEconomyService.js';
...
export class SocketServer {
    ...
    private setupHandlers() {
        ...
        this.io.on('connection', (socket: Socket) => {
            ...
            // Room join / switch listener
            socket.on('room:join', (payload: { roomId: string }) => {
                if (payload?.roomId) {
                    socket.join(payload.roomId);
                    roomId = payload.roomId;
                    Logger.info(`👤 User ${user?.name || 'Guest'} joined/switched to Room: ${roomId}`, 'SocketServer');
                    if (user) this.broadcastRoomUsers(roomId);
                }
            });
            ...
            socket.on('stream:relay', (payload: { sessionId: string, chunk: Buffer }) => {
                const success = streamingService.ingestRelayChunk(payload.sessionId, payload.chunk);
                if (success === false) {
                     socket.emit('stream:error', { sessionId: payload.sessionId, reason: 'Relay ingestion failed (FFmpeg crashed or session not found).' });
                }
            });
            ...
            // Client-Side Chat Sync Ingestion
            socket.on('client:chat:sync', (payload: { username: string, text: string }) => {
                if (roomId && payload && payload.text) {
                    import('../ai/LiveSalesServiceV3.js').then(({ liveSalesServiceV3 }) => {
                        liveSalesServiceV3.feedChatMessage(roomId, payload.username || 'Viewer', payload.text);
                    });
                    // Relay to other connected sockets in the room
                    socket.to(roomId).emit('chat:external:relayed', {
                    ...
                    if (roomId) {
                        ...
                        if (isFullyDisconnected) {
                            ...
                            const timer = setTimeout(() => {
                                Logger.info(`💀 [SocketServer] Socket grace period expired for user ${user.name}. Auto-stopping room: ${roomId}...`, 'SocketServer');
                                this.io?.to(roomId).emit('guest:leave', { guestId: user.id });
                                this.broadcastRoomUsers(roomId);
                                this.disconnectGracePeriods.delete(user.id);

                                // Terminate active stream and autonomous FSM for this room
                                streamingService.stopRestream(roomId).catch(e => {
                                    Logger.warn(`[SocketServer] Error stopping stream on grace period expiry for ${roomId}: ${e}`);
                                });
                                import('../ai/LiveSalesServiceV3.js').then(({ liveSalesServiceV3 }) => {
                                    liveSalesServiceV3.stopFSM(roomId);
                                });
                            }, 60000); // 60 seconds
    ...
    public async getRoomSocketCount(roomId: string): Promise<number> {
        if (!this.io) return 0;
        try {
            const sockets = await this.io.in(roomId).fetchSockets();
            return sockets.length;
        } catch (error: any) {
            Logger.error(`[SocketServer] Error getting room socket count for ${roomId}: ${error.message}`, 'SocketServer');
            return 0;
        }
    }
