...
//import { notificationService } from '../system/NotificationService.js';
    ...
    public async syndicateFinalVideo(projectId: string, metadata: { caption: string, hashtags: string[] }) {
        try {
            ...
            const accounts = await UserPlatformAccount.find({
                ...
                isActive: true,
                platform: { $in: [SocialPlatform.TIKTOK, SocialPlatform.YOUTUBE, SocialPlatform.FACEBOOK] }
            });
        ...
        try {
            ...
            const results = await Promise.all(accounts.map(async (acc) => {
                ...
                try {
                    ...
                    const videoStream = await storage.getFileStream(s3Key);
                    const ext = s3Key.endsWith('.webm') ? '.webm' : '.mp4';
