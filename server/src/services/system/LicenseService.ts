...
import { License, LicenseStatus } from '~/models/License.js';
...
export class LicenseService {
     ...
     */
    static start() {
        if (configService.isMasterServer) return;
    ...
    static async getLicense() {
        ...
        if (settings && settings.license && settings.license.key && settings.license.info && settings.license.info.status === LicenseStatus.VALID) {
            this.currentLicense = {
    ...
    public static async checkStatus(forcedKey?: string) {
        try {
            ...
            Logger.info(`LicenseWorker: Validating license [${key.substring(0, 10)}...]`, "LicenseWorker", {
                ...
                version: '1.0.0',
                masterServerUrl: EnvConfig.masterServerUrl
            });
            ...
            if (remote.status === LicenseStatus.VALID) {
                ...
                if (settings) {
                    settings.license.info.status = (remote.status || LicenseStatus.EXPIRED) as any;
                ...
                this.currentLicense = {
                    key: key,
                    status: remote.status || LicenseStatus.EXPIRED,
