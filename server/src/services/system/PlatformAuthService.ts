...
export class PlatformAuthService {
     ...
     */
    private static async getConfig(platform: any) {
        ...
        switch (platform) {
            ...
            // case SocialPlatform.GOOGLE:
                ...
                return {
                    ...
                    redirectUri: oauth?.google?.redirectUri || `${baseUrl}/platforms/callback/youtube`,
            ...
            case SocialPlatform.FACEBOOK:
                ...
                return {
                    ...
                    redirectUri: oauth?.facebook?.redirectUri || `${baseUrl}/platforms/callback/facebook`,
            ...
            case SocialPlatform.TIKTOK:
                ...
                return {
                    ...
                    clientSecret: oauth.tiktok.clientSecret || '',
                    redirectUri: oauth.tiktok.redirectUri || `${baseUrl}/platforms/callback/tiktok`,
    ...
    /**
     * Generate OAuth Login URL for a platform
     */
    static async getAuthUrl(platform: any): Promise<string> {
     ...
     */
    static async exchangeCode(platform: any, code: string): Promise<any> {
        const config = await this.getConfig(platform);
     ...
     * Refresh OAuth Token
     */
    static async refreshOAuthToken(platform: any, refreshToken: string): Promise<any> {
        const config = await this.getConfig(platform);
     ...
     */
    static async getVideos(platform: any, credentials: any, options: { page?: number, limit?: number, search?: string, sort?: string } = {}): Promise<{ videos: any[], total: number }> {
     ...
     */
    static async getComments(platform: any, credentials: any, videoId: string): Promise<any[]> {
     ...
     */
    static async postComment(platform: any, credentials: any, videoId: string, text: string, parentId?: string): Promise<any> {
        const { accessToken } = credentials;
     ...
     */
    static async getStats(platform: any, credentials: any): Promise<any> {
        const { accessToken } = credentials;
     ...
     * Delete Video
     */
    static async deleteVideo(platform: any, credentials: any, videoId: string): Promise<boolean> {
        const { accessToken, serverUrl, email, password, appName } = credentials;
     ...
     */
    static async uploadVideo(platform: any, credentials: any, videoStream: any, metadata: any, filename?: string, contentType?: string): Promise<any> {
        const { accessToken, serverUrl, email, password, appName } = credentials;
     ...
     */
    static async getLiveStreamInfo(platform: any, credentials: any, metadata: { title: string, description: string }): Promise<{ rtmpUrl: string, streamKey: string, externalChatId?: string, externalId?: string }> {
        const { accessToken, serverUrl, email, password, appName } = credentials;
     ...
     */
    static async updateLiveStreamMetadata(platform: any, credentials: any, metadata: { title: string, description: string }): Promise<boolean> {
        const { accessToken, serverUrl, email, password, appName } = credentials;
