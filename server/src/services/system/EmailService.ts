...
export class EmailService {
    ...
    public async sendWelcomeEmail(user: { email: string; name: string }) {
        const subject = 'Welcome to AntStudio!'
        const html = `
            <div style="font-family: Arial, sans-serif; color: #333;">
                <h2>Welcome, ${user.name}!</h2>
                <p>Thank you for joining AntStudio. We are excited to have you on board.</p>
                <p>Start exploring our AI-powered features today!</p>
                <br>
                <p>Best regards,</p>
                <p>The AntStudio Team</p>
            </div>
        `
    ...
    public async sendPasswordResetEmail(user: { email: string; name: string }, token: string) {
        const resetLink = `${configService.publicDomain}/reset-password?token=${token}`
