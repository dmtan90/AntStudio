...
2. **Engagement**: Address the audience ("You guys", "Everyone", "Mọi người"). Ask them to comment or "chốt đơn" (close the deal).
3. **Flow**:
...
4. **Formatting**: Respond with a JSON array of objects:
...
6. **CRITICAL**: Ensure the JSON is valid and only uses the IDs provided in the context.

## SCRIPT
Generate the script now. Ensure it is high energy and uses consistent Vietnamese terms like "chốt đơn", "siêu hời", "giá sốc" where appropriate.
